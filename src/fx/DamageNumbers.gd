...
static func spawn(world_pos: Vector2, damage: int, is_crit: bool = false) -> void:
	var container := DamageNumbers.get_or_create_container()
	
	var label := Label.new()
	label.text = str(damage)
	label.add_theme_font_size_override("font_size", is_crit ? 24 : 16)
	label.mod = Color(1.0, 0.3, 0.3, 1.0) if not is_crit else Color(1.0, 0.9, 0.2, 1.0)
	label.z_index = 200
	...
	tween.tween_property(label, "position:y", world_pos.y - float_distance + offset.y, float_duration).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property(label, "mod:a", 0.0, float_duration).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property(label, "scale", Vector2(end_scale, end_scale), float_duration).set_trans(Tween.TRANS_LINEAR)
...
static func get_or_create_container() -> CanvasLayer:
	var existing := Engine.get_main_loop().root.get_node_or_null("DamageNumbersLayer")
	if existing:
		return existing
