...
func _ready() -> void:
	...
	_validate_player_self_shadow_disabled(tower.player, failures)
	_finish(failures)
...
func _validate_player_self_shadow_disabled(player: Player3D, failures: Array[String]) -> void:
	var caster_count := 0
	...
	if player.weapon != null:
		for mesh_value in player.weapon.find_children("*", "MeshInstance3D", true, false):
			...
			if mesh.cast_shadow != GeometryInstance3D.SHADOW_CASTING_SETTING_OFF:
				caster_count += 1
	if caster_count != 0:
		failures.append("player/avatar/held weapon still contains %d shadow casters" % caster_count)


func _finish(failures: Array[String]) -> void:
	if failures.is_empty():
		print("TOWER_GRID_COMPONENT_ALIGNMENT_OK: 5m bounds, real door meshes, corridors, floor coverage and player self-shadow pass")
		get_tree().quit(0)
		return
