extends Node
## PH49 98—95层搜打撤、30m基地、独立电梯、全局光照、窄门与全息HUD验收。
...
func _ready() -> void:
	...
	_expect(
		tower.player.camera.position.is_equal_approx(Vector3(0.0, 8.0, 5.5)),
		"塔楼摄像机没有保持Y=8m、后移5.5m",
		failures
	)
	_expect(
		tower.player.camera.rotation.x > -1.2
		and tower.player.camera.rotation.x < -0.7,
		"塔楼摄像机没有形成室内斜俯视角",
		failures
	)
	_expect(
...
func _validate_camera_lower_wall_lift(
	...
	blocker.global_position = tower.player.global_position + Vector3(0.0, 4.5, 2.4)
	...
	_expect(
		retract_is_smooth
		and trailing_samples[0] - trailing_samples[trailing_samples.size() - 1] > 2.5,
		"下方墙镜头收拢不是连续平滑过程",
		failures
	)
	_expect(
	...
	_expect(
		...
		and absf(tower.player.camera.position.z - 5.5) <= 0.03,
		"下方墙移除后镜头或角色轮廓没有平滑恢复",
		failures
	)
...
func _finish(failures: Array[String], node_count: int) -> void:
	if failures.is_empty():
		print("TOWER_DESCENT_FLOW_OK: PH49基地、98—95搜打撤、独立电梯、全局光照、窄门、墙体与全息小地图通过（nodes=%d）" % node_count)
		get_tree().quit(0)
		return
