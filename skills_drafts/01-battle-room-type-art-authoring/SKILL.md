...
description: 当制作或深化 ShellStorm2 战局中的通用房、安全房、BOSS房、撤离房等房间种类 Blender 美术源时使用。开始正式建模前调用 02 冻结不超过 50 个组件的计划，再以组件母版和实例完成房型源；不修改玩法或程序化房间规则。
agent_created: true
metadata:
...
严格调用 `blender-game-prop-standard`，并把 `02-battle-room-component-decomposer` 的“制作前组件规划”嵌入同一轮制作：

1. 先建立白模与范围快照，记录尺寸、墙厚、逻辑高度、可见高度、门洞和连接方向。
2. **建正式几何前先执行 02 的组件拆分判断**：从概念图列出 `component_plan.json`，冻结唯一组件定义；单个房型组件定义总数必须 `<= 50`，实例数量不限。
3. 每个计划项只建一份组件母版；同件重复出现时只放 Collection Instance。房型源必须从第一天就是“组件母版＋实例布局”，禁止先焊成整屋、验收后再拆。
4. 同族常规变体目标 `<= 3`；只有明确的破损、配色或结构状态轴允许扩到 `4–5` 个，族内每件都在计划中记录同一语义的 `variant_axis` 及各自 `variant_value` / `variant_reason`。超过 5 个失败。
5. 按效果图迭代时，修改母版会同步所有实例；确需新增造型时先更新组件计划和预算，再建几何。墙、地板、天花、门框、固定设施、管线和环境支持都须保留语义边界。
6. 所有独立设施、建筑模块、地砖、门和撤离信标必须能单独识别；不得把全屋合并成一个不可拆解网格。
7. 使用项目公共色盘 `assets/art/shared/palette/设施低亮多巴胺色盘_10x10_512.png`，仅使用四个共享材质角色，保留 PaletteUV。
8. 保留 `01_制作组件` 与 `02_游戏输出` 的层级；`01_制作组件` 放母版，`02_游戏输出` 只放计划内实例。展示相机、灯光和参考辅助对象不得进入游戏组件输出。
9. 保存新版本，不覆盖历史源；房间种类源统一放置于：
...
门禁：尺寸/门洞/墙高不符合白模则失败；没有在建模前冻结 02 的组件计划则失败；唯一组件定义 >50、族内变体无理由 >3 或任何族 >5 则失败；房间只有整屋网格、重复复制 Mesh 而不是母版实例则失败；材质、PaletteUV、色盘路径不符合规范则失败；新增玩法或规则代码则失败。完成后仍由 `02-battle-room-component-decomposer` 做组件包、实例清单与双端还原的收尾验收，但不再事后重新决定如何拆。
