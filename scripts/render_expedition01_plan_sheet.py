...
# 业主 2026-09-25 裁定：**支线整体取消** —— 本关不再有 branch_01..04，图纸只画主路 13 房
# （安全屋 + 10 间内容房 + Boss + 撤离）。设计页的支线设计已作废（§2 顶部裁定 / §3.3 / §5.1），
# 主路与坐标已按 13 房重排并落到设计源、模板与验收断言（2026-09-25 第三轮）。
...
# 序号 r["i"] 按 §3.1 总表原始序（本关 1..13 即主路 13 房）。
...
# ✅ 2026-09-25 第三轮：本表与 `room_templates/*.json` 的 `variant_footprints` **已逐值一致**
# （门禁 `check_expedition_room_footprints.py` 校 JSON；`room_06` 由旧 50×30 收敛为 40×30 = db_02）。
# 同一房型在主路的多个实例共用同一份顶点（room_01/room_08 = l_turn、room_02/room_10 = db_01）。
OUTLINES = {
    # 拐角走廊 L 形（`corridor_45x40::l_turn`）：横臂 45×15 + 竖臂 15×25
    "room_01": [(0, 0), (45, 0), (45, 15), (15, 15), (15, 40), (0, 40)],
    "room_08": [(0, 0), (45, 0), (45, 15), (15, 15), (15, 40), (0, 40)],
    # 拐角走廊 凹字形折返（`corridor_45x40::u_turn`）：上下横臂 45×15 + 右侧竖 15×10
    "room_04": [(0, 0), (45, 0), (45, 40), (0, 40), (0, 25), (30, 25), (30, 15), (0, 15)],
    # 数据库房间01 40×30（`db_70x50::db_01`）：主体 40×25 + 南侧中段外凸 10×5
    "room_02": [(0, 0), (40, 0), (40, 25), (25, 25), (25, 30), (15, 30), (15, 25), (0, 25)],
    "room_10": [(0, 0), (40, 0), (40, 25), (25, 25), (25, 30), (15, 30), (15, 25), (0, 25)],
    # 数据库房间02 40×30（`db_70x50::db_02`）：左段 15×20 + 中段 10×30 + 右段 15×25
    # （相对 40×30 包围盒 = 左下缺 15×10、右下缺 15×5、南中段外凸 10×5）
    "room_06": [(0, 0), (40, 0), (40, 25), (25, 25), (25, 30), (15, 30), (15, 20), (0, 20)],
    # 通道桥房间 30×60 工字型（`bridge_60x50`，模板本身无 variant_footprints，靠 sunken_pit + bridge_span 表达）：
    # 南北两端 30×15 上层平台 + 中间 10 m 宽跨桥（x 10~20）
...
def build_overview(with_grid=True):
    ...
    o.append(text(lx + 34, ly + 84, f"主路行进方向（①→⑬ 穿墙 · {len(MAIN_SEQUENCE)} 房）", 9.5, "start", DIM))
...
def build_details():
    ...
    # 2026-09-25 按业主指示由 70×50 缩到 40×30 —— 轮廓顶点全部落 5 m 网格，与
    # room_templates/db_70x50.json 的 variant_footprints.db_01 **逐值一致**。
    # ⚠️ db_01 **不是缺角形态**：主体 40×25 + 南中段外凸 10×5（x∈[15,25] 居中），
    #    四个角都是直角；别照着 db_02（缺角）去写它的说明。
    def db1(X, Y):
        k = KIND["COMMON"]
    ...
    # ---- 数据库房间02 40x30（不规则：左段 15×20 + 中段 10×30 + 右段 15×25）
    # 2026-09-25 按业主指示由 70×50 缩到 40×30，轮廓 = room_templates/db_70x50.json 的
    # variant_footprints.db_02（逐值一致）：左下缺 15×10、右下缺 15×5、南中段外凸 10×5。
    def db2(X, Y):
        k = KIND["COMMON"]
        walk = [(0, 0), (40, 0), (40, 25), (25, 25), (25, 30), (15, 30), (15, 20), (0, 20)]
        g = [poly([(X(a), Y(b)) for a, b in walk], k["fill"], k["stroke"], 1.2)]
        for row in range(3):
            for col in range(4):
                g.append(rect(X(7 + col * 7.6), Y(4 + row * 6.2), 5.4, 3.2, "#cbdcf0", k["stroke"], 0.6, 1))
        g.append(rect(X(1.2), Y(9), 3.6, 6, "#FAC775", "#BA7517", 0.8, 1))
        g.append(label(X, Y, 3, 17, "叉车", 8, "#633806"))
        g.append(label(X, Y, 22, 11, "货架 · 箱体堆场", 9.5, k["ink"], "middle", "500"))
        g.append(foot_label(X, Y, dict(w=40, d=30),
                            "40 × 30 m · 不规则（左缺 15×10 · 右缺 15×5 · 南中段外凸 10×5）"))
        return g
    out["room_06"] = db2

    # ---- 标准房间 25x25（最简房型：四墙平房 + 空内容，家具/设备靠 RuntimeDetail 流送）
    def std(X, Y):
        k = KIND["COMMON"]
        g = [rect(X(0), Y(0), 25 * DS, 25 * DS, k["fill"], k["stroke"], 1.2, 2)]
        g.append(rect(X(1.4), Y(1.4), 22.2 * DS, 22.2 * DS, "none", k["stroke"], 0.6, 3, "5 4"))
        g.append(label(X, Y, 12.5, 12.5, "空内容", 9.5, DIM))
        g.append(label(X, Y, 12.5, 17.0, "（RuntimeDetail 流送）", 8, DIM))
        g.append(foot_label(X, Y, dict(w=25, d=25), "25 × 25 m · 四墙平房 · 无内墙"))
        return g
    out["room_07"] = std

    # ---- room_08 与 room_01 同房型（拐角走廊 L 形）⇒ 复用同一份绘制函数
    out["room_08"] = c1

    # ---- room_09 与 room_03 同房型（办公室 30×40）⇒ 复用同一份绘制函数
    out["room_09"] = off

    # ---- room_10 与 room_02 同房型（数据库房间01 40×30）⇒ 复用同一份绘制函数
    out["room_10"] = db1

    # ---- 通道桥房间 30x60 工字型（南北两端上层平台 + 中央跨桥，桥两侧为下层）
    # 2026-09-25 按业主指示改画：上层平台俯视呈「工」字 ——
    ...
    # ✅ room_templates/bridge_60x50.json **已同步为同一形态**：`size_m = [30, 60]`、
    #    `sunken_pit` 30×30（y 15~45）、`bridge_span` 10 m 宽跨桥（x 10~20）；模板 id 名嵌的旧尺寸按裁定锁定不改名。
    def bridge(X, Y):
        k = KIND["COMMON"]
        g = []
    ...
    # 注：13 房示例版图里 10 间内容房用到 5 个 `COMMON_ROOM` 模板 ⇒ 绘制函数覆盖全部 5 个房型
    #     （corridor L/凹 + db_01/db_02 + office + bridge + std）。同房型的多个实例复用同一函数
    #     （room_08=room_01、room_09=room_03、room_10=room_02）。若 §3.4 池子将来新增房型，
    #     这里要补一份绘制函数，否则重跑会 KeyError（出图不做静默兜底）。
...
def build_html():
    ...
    for r in DRAWN_ROOMS:
        extra = {
            ...
            "room_02": "主路房 2 · 房型 数据库房间01（40 × 30，南中段外凸 10 × 5）",
            "room_03": "主路房 3 · 房型 办公室（30 × 40，无内墙）",
            "room_04": "主路房 4 · 房型 拐角走廊 凹字折返",
            "room_05": "主路房 5 · 房型 通道桥房间（30 × 60 工字型）· 两扇门是桥的两端 · 唯一多层几何",
            "room_06": "主路房 6 · 房型 数据库房间02（40 × 30 缺角：左缺 15 × 10 · 右缺 15 × 5）",
            "room_07": "主路房 7 · 房型 标准房间（25 × 25，四墙平房、空内容）",
            "room_08": "主路房 8 · 房型 拐角走廊 L 形（与 room_01 同房型）",
            "room_09": "主路房 9 · 房型 办公室（30 × 40，与 room_03 同房型）",
            "room_10": "主路房 10 · 房型 数据库房间01（40 × 30，与 room_02 同房型）· 示例版图最后一间内容房",
...
<h2>1. 总平面图</h2>
<p class="lede">主路：{' → '.join(BY_KEY[k]["name"] for k in MAIN_SEQUENCE)}。<b>十二段衔接全部墙贴墙</b>（净距 0 = 共墙），不再插过渡走廊模块。<b>支线已取消</b>（业主 2026-09-25 裁定）—— 场上不再有支线房，图纸只画这条主路。<b>主路已落到 13 房口径</b>（安全屋 + 10 间内容房 + Boss + 撤离），房型模板 JSON / <code>floor_00.json</code> / 验收断言 / 本图集四处已同轮改完（设计页 §2 顶部裁定）。画面向上为北（平面 +y = 世界 +z = 南）。</p>
...
<h2>2. 主路与门数</h2>
{flow_html}
<p class="lede" style="margin-top:14px">主路 {len(MAIN_SEQUENCE) - 1} 段衔接全部为 <b>墙贴墙（净距 0）</b>、共轴偏移最大 <b>5.0 m</b>；每段走廊的过渡由共享墙上的门洞完成，不再有独立的走廊模块。门数：入口安全屋 <b>2</b>（前门 + 弃局门）；<b>主路内容房每间 2</b>（进 + 出）—— 支线取消后本关不再有 3 门房；Boss 竞技场 <b>2</b>；撤离屋 <b>1</b>。图上只标每条主路边的门位。</p>
...
<h2>4. 面积构成</h2>
{bars_html}
<p class="cap" style="margin-top:10px;color:var(--muted);font-size:12px">房间面积 {n(tot)} m² ｜ 内墙估算 {n(wall)} m² ｜ 走廊估算 {n(corridor)} m²（墙贴墙 ⇒ 0）｜ 估算占用 <b>{n(used)} m²</b>。上列为<b>主路 13 房</b>的数字（支线已取消，场上无支线房）。<b>面积自 2026-09-25 起全项目只报数据、不设上限</b>（场地外边界同时退出判据）—— 这些数字仅供对照，不参与准入判定，见设计页 §4.7。</p>
...
<li><b>✅ 尺寸已四处同步（2026-09-25 第三轮）。</b>`room_02` / `room_03` / `room_05` / `room_06` 四间的尺寸与形态（含桥房工字型）连同 <code>room_templates/*.json</code> 的 <code>size_m</code> / <code>variant_footprints</code> / <code>sunken_pit</code> / <code>bridge_span</code>、<code>floor_00.json</code> 的 13 房表与验收断言 <b>同轮改完</b> —— 本图与模板 JSON 的非矩形轮廓<b>逐值一致</b>（门禁 <code>check_expedition_room_footprints.py</code> 校 JSON）。</li>
...
<li><b>Boss 房门位由生成算法定。</b>墙贴墙摆位会给 Boss 房两个贴合方向（进 / 出）；本图按坐标草案记的是<b>西进南出</b>（西接数据库房间01 = <code>room_10</code>、南接撤离屋），美术源的门洞须与生成结果对齐。</li>
...
生成于 2026-09-25 · 示意图集（画的是<b>示例版图</b>；实跑版图由 <code>FloorPlanGenerator</code> 按种子现算）。本版：四房缩尺 + 桥房改工字型 + 主路加深到 13 房 + 支线取消（图纸只画主路）。
</div>
