...
PACKAGE=BASE/'v016'; OUT=ROOT/'outputs/character_pipeline/v016'
MODEL=PACKAGE/'source/model/chr_bunny01_model_v016.blend'
TARGET=PACKAGE/'source/animation/chr_bunny01_animation_v016.blend'
assert not TARGET.exists() or '--replace-generated' in sys.argv,'Preserve existing artist work'
bpy.ops.wm.open_mainfile(filepath=str(BASE/'v015/source/animation/chr_bunny01_animation_v015.blend'))
...
for scene in list(bpy.data.scenes):
    ...
    if state in ['moving','armed_moving']:
        ...
        action=bpy.data.actions.new('anim_bunny01_'+state+'_v016')
        ...
        for i in range(period+1):
            ...
            head_delta=tr((.004*math.cos(w-.25),.014,.026+.032*(.5-.5*math.cos(2*w-.18))))@Matrix.Translation(head_pivot)@tr(r=(.12+.015*math.sin(2*w-.4),0,0))@Matrix.Translation(-head_pivot)
        ...
        old.name=old.name.replace('_v015','_v016')
    ...
    scene['model_source']='../model/chr_bunny01_model_v016.blend'
    scene.frame_set(1)
...
shutil.copy2(BASE/'v015/source/model/chr_bunny01_model_v015.blend',MODEL)
for lib in bpy.data.libraries:
    if 'chr_bunny01_model' in lib.filepath: lib.filepath='//../model/chr_bunny01_model_v016.blend'
...
report={'skeleton_sha256':sig,'model_unchanged':hashlib.sha256(MODEL.read_bytes()).hexdigest()==hashlib.sha256((BASE/'v015/source/model/chr_bunny01_model_v015.blend').read_bytes()).hexdigest(),
...
ledger={'asset_id':'CHR-PLY-CAPSULE01-3D-BUNNY01','version':'v016','status':'authored_pending_export','runtime_active_version':'v011',
...
(PACKAGE/'character_transfer_ledger_v016.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2))
print('V016_LIVELIER_JOG_VALIDATED',TARGET)
