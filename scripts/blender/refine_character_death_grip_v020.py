...
PACKAGE=BASE/'v020'; OUT=ROOT/'outputs/character_pipeline/v020'
MODEL=PACKAGE/'source/model/chr_bunny01_model_v020.blend'
TARGET=PACKAGE/'source/animation/chr_bunny01_animation_v020.blend'
...
delta=gun.matrix_world.to_3x3()@(live_mesh_location-mesh.location)
...
death_keys=[key(0),key(.08,p=-.24,z=-.06,hp=-.12,al=-.45,ar=-.45,ear=.28),
 ...
 key(.66,spread=-.12,roll=-2.5*math.pi,ear=.06),
 key(.75,spread=-.14,roll=-2.5*math.pi+.07,ear=-.08,air=.075),
 key(.86,spread=-.12,roll=-2.5*math.pi),key(1,spread=-.12,roll=-2.5*math.pi)]
...
for scene in list(bpy.data.scenes):
    ...
    if state not in changed:
        old.name=old.name.replace('_v019','_v020'); continue
    ...
    action=bpy.data.actions.new('anim_bunny01_'+state+'_v020'); rig.animation_data.action=action
    previous={}
    ...
    scene.frame_set(1)
for scene in bpy.data.scenes: scene['model_source']='../model/chr_bunny01_model_v020.blend'
...
for lib in bpy.data.libraries:
    if 'chr_bunny01_model' in lib.filepath: lib.filepath='//../model/chr_bunny01_model_v020.blend'
...
ledger={'asset_id':'CHR-PLY-CAPSULE01-3D-BUNNY01','version':'v020','status':'authored_pending_export','runtime_active_version':'v011','skeleton_id':'SKEL-BUNNY01-004','validation':report,
...
(PACKAGE/'character_transfer_ledger_v020.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2))
print('V020_VALIDATED',TARGET)
