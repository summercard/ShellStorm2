...
src=(R/'deepen.py').read_text();tree=ast.parse(src);node=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='sig');exec(ast.get_source_segment(src,node))
