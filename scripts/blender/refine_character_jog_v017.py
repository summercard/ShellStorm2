...
PACKAGE=BASE/'v017'; OUT=ROOT/'outputs/character_pipeline/v017'
MODEL=PACKAGE/'source/model/chr_bunny01_model_v017.blend'
TARGET=PACKAGE/'source/animation/chr_bunny01_animation_v017.blend'
assert not TARGET.exists() or '--replace-generated' in sys.argv,'Preserve existing artist work'
bpy.ops.wm.open_mainfile(filepath=str(BASE/'v016/source/animation/chr_bunny01_animation_v016.blend'))
...
for scene in list(bpy.data.scenes):
    ...
    if state in ['moving','armed_moving']:
        ...
        action=bpy.data.actions.new('anim_bunny01_'+state+'_v017')
        ...
        for i in range(period+1):
            ...
            for side,sign in [('l',-1),('r',1)]:
                ...
                if armed and side=='r':
                    ...
                    rotation=tr(r=(swing,0,0))@tr(r=(0,sign*.88,0))
        ...
        old.name=old.name.replace('_v016','_v017')
    ...
    scene['model_source']='../model/chr_bunny01_model_v017.blend'
    scene.frame_set(1)
...
shutil.copy2(BASE/'v016/source/model/chr_bunny01_model_v016.blend',MODEL)
for lib in bpy.data.libraries:
    if 'chr_bunny01_model' in lib.filepath: lib.filepath='//../model/chr_bunny01_model_v017.blend'
...
report={'skeleton_sha256':sig,'model_unchanged':hashlib.sha256(MODEL.read_bytes()).hexdigest()==hashlib.sha256((BASE/'v016/source/model/chr_bunny01_model_v016.blend').read_bytes()).hexdigest(),
...
ledger={'asset_id':'CHR-PLY-CAPSULE01-3D-BUNNY01','version':'v017','status':'authored_pending_export','runtime_active_version':'v011',
...
(PACKAGE/'character_transfer_ledger_v017.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2))
print('V017_LIVELIER_JOG_VALIDATED',TARGET)
