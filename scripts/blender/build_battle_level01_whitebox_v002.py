"""Build Battle level 01 whitebox components as independently editable meshes.

Run with:
    blender --background --python scripts/blender/build_battle_level01_whitebox_v002.py

Version v002 fixes the v001 merge regression. Walls, floor slabs, floor tiles,
door-side wall pieces, lintels and corridor details remain separate objects in
both the authoring and game-output collections.
"""
...
OUTPUT_ROOT = ROOT / "source/art/whitebox/tower_zones/battle_level01/v002"
...
def create_side_walls(
    ...
    left_edge = -lateral_size * 0.5
    right_edge = lateral_size * 0.5
    for index, (opening_center, _) in enumerate(openings):
        opening_left = opening_center - DOOR_WIDTH * 0.5
        opening_right = opening_center + DOOR_WIDTH * 0.5
        if index == 0:
            name = f"WALL_{side_upper}_DOOR_{index + 1:02d}_LEFT"
            role = f"{side_upper} 门洞{index + 1:02d} 左侧墙"
            if is_horizontal:
                add_horizontal_segment(name, left_edge, opening_left, role)
            else:
                add_vertical_segment(name, left_edge, opening_left, role)
        else:
            previous = openings[index - 1][0]
            name = f"WALL_{side_upper}_BETWEEN_DOOR_{index:02d}_{index + 1:02d}"
            role = f"{side_upper} 门洞{index:02d}与{index + 1:02d}之间墙"
            if is_horizontal:
                add_horizontal_segment(
                    name,
                    previous + DOOR_WIDTH * 0.5,
                    opening_left,
                    role,
                )
            else:
        ...
        if is_horizontal:
            objects.append(
                create_box(
                    f"WALL_{side_upper}_DOOR_{index + 1:02d}_LINTEL",
                    (
                        opening_center,
                        wall_axis,
                        DOOR_TOP + lintel_height * 0.5,
                    ),
                    ...
                    (9, 6),
                    "door_wall",
                    f"{side_upper} 门洞{index + 1:02d} 门楣",
                    "structure",
                )
            )
        else:
    ...
    last_index = len(openings)
    last_center = openings[-1][0]
    right_segment_name = f"WALL_{side_upper}_DOOR_{last_index:02d}_RIGHT"
    right_segment_role = f"{side_upper} 门洞{last_index:02d} 右侧墙"
    if is_horizontal:
        add_horizontal_segment(
            right_segment_name,
            last_center + DOOR_WIDTH * 0.5,
            right_edge,
            right_segment_role,
        )
    else:
...
def build_asset(asset: dict[str, object]) -> dict[str, object]:
    ...
    manifest = {
        ...
        "component_policy": "independent_meshes; walls, floor slabs, tiles and door walls are never joined",
    ...
    print(
        "BATTLE_LEVEL01_WHITEBOX_V002_ASSET_OK",
        json.dumps(
            {
                "asset_id": asset_id,
...
def write_catalog(manifests: list[dict[str, object]]) -> None:
    ...
    catalog = {
        ...
        "component_policy": "独立墙体、地板、地砖和门洞墙；禁止 join",
...
def main() -> None:
    ...
    print(
        "BATTLE_LEVEL01_WHITEBOX_V002_READY",
        json.dumps(
            {
