...
PACKAGE=BASE/'v014'; OUT=ROOT/'outputs/character_pipeline/v014'
MODEL=PACKAGE/'source/model/chr_bunny01_model_v014.blend'
TARGET=PACKAGE/'source/animation/chr_bunny01_animation_v014.blend'
assert not TARGET.exists() or '--replace-generated' in sys.argv,'Preserve existing artist work'
bpy.ops.wm.open_mainfile(filepath=str(BASE/'v013/source/animation/chr_bunny01_animation_v013.blend'))
...
for scene in list(bpy.data.scenes):
    ...
    if state in ['moving','armed_moving']:
        ...
        action=bpy.data.actions.new('anim_bunny01_'+state+'_v014')
        ...
        for i in range(period+1):
            ...
            pelvis=tr((.012*math.cos(w),.006,.006+.033*pulse),(-.045,.032*math.cos(w),.04*math.cos(w)))
            chest=pelvis@tr(r=(.014*math.sin(2*w-.25),0,-.065*math.cos(w)))
            posed['waist']=pelvis@rest['waist']; posed['chest']=chest@rest['chest']
            head_delta=tr((.007*math.cos(w-.25),.005,.010+.025*(.5-.5*math.cos(2*w-.18))),(-.015+.012*math.sin(2*w-.4),.014*math.cos(w-.2),-.018*math.cos(w)))
            ...
            for side,sign in [('l',-1),('r',1)]:
                ...
                if phase<.40:
                    ...
                    foot.y+=-.12+.24*ease
                    foot.z+=.090*math.sin(math.pi*u)**2
                ...
                ear='ear_'+side
                posed[ear]=head_delta@rest[ear]@tr(r=(.15*math.sin(2*w-.85+sign*.16),sign*.035*math.cos(w-.3),sign*.075*math.sin(2*w-1.15)))
        ...
        old.name=old.name.replace('_v013','_v014')
    ...
    scene['model_source']='../model/chr_bunny01_model_v014.blend'
    scene.frame_set(1)
...
shutil.copy2(BASE/'v013/source/model/chr_bunny01_model_v013.blend',MODEL)
for lib in bpy.data.libraries:
    if 'chr_bunny01_model' in lib.filepath: lib.filepath='//../model/chr_bunny01_model_v014.blend'
...
report={'skeleton_sha256':sig,'model_unchanged':hashlib.sha256(MODEL.read_bytes()).hexdigest()==hashlib.sha256((BASE/'v013/source/model/chr_bunny01_model_v013.blend').read_bytes()).hexdigest(),
...
ledger={'asset_id':'CHR-PLY-CAPSULE01-3D-BUNNY01','version':'v014','status':'authored_pending_export','runtime_active_version':'v011',
...
(PACKAGE/'character_transfer_ledger_v014.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2))
print('V014_SHOULDER_FK_AND_PALM_GRIP_VALIDATED',TARGET)
