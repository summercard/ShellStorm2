...
PACKAGE=BASE/'v018'; OUT=ROOT/'outputs/character_pipeline/v018'
MODEL=PACKAGE/'source/model/chr_bunny01_model_v018.blend'
TARGET=PACKAGE/'source/animation/chr_bunny01_animation_v018.blend'
assert not TARGET.exists() or '--replace-generated' in sys.argv,'Preserve existing artist work'
bpy.ops.wm.open_mainfile(filepath=str(BASE/'v017/source/animation/chr_bunny01_animation_v017.blend'))
...
for scene in list(bpy.data.scenes):
    ...
    if state in ['moving','armed_moving']:
        ...
        action=bpy.data.actions.new('anim_bunny01_'+state+'_v018')
        ...
        for i in range(period+1):
            ...
            head_pivot=rest['head'].translation
            head_delta=tr((.004*math.cos(w-.25),.150,.026+.032*(.5-.5*math.cos(2*w-.18))))@Matrix.Translation(head_pivot)@tr(r=(.12+.015*math.sin(2*w-.4),0,0))@Matrix.Translation(-head_pivot)
            ...
            for side,sign in [('l',-1),('r',1)]:
                ...
                if armed and side=='r':
                    ...
                    swing=sign*1.13*math.cos(w)
                    # Lower the whole arm into its running plane FIRST, then
                    # swing that plane about the shoulder's transverse axis.
                    # Open the running plane outward by 12.6 degrees at the
                    # shoulder; retain full FK swing, never translate the hand.
                    rotation=tr(r=(swing,0,0))@tr(r=(0,sign*.66,0))
                    basis=rest[upper].to_3x3().to_4x4()
                    posed[upper]=chest@rest[upper]@basis.inverted()@rotation@basis
                    basis=rest[lower].to_3x3().to_4x4()
                    elbow_flex=.10+.10*math.sin(w+sign*.6)
                ...
                pitch=0.0
                if phase<.36:
                    u=phase/.36; ease=u*u*(3-2*u)
                    foot.y+=.16-.32*ease
                    # Heel rises over the planted toe during terminal push-off.
                    push=max(0.0,(u-.55)/.45)
                    pitch=-.48*push*push*(3-2*push)
                else:
                    u=(phase-.36)/.64; ease=u*u*(3-2*u)
                    foot.y+=-.16+.32*ease
                    foot.z+=.145*math.sin(math.pi*u)**2
                    # Toe trails after push-off, then toes lift in preparation
                    # for landing. Smooth endpoints keep the loop continuous.
                    pitch=-.48*(1-u)**2+.40*math.sin(math.pi*u)**2
                rotation=tr(r=(pitch,0,0))
                # Keep the sole above ground while pitching the rigid shoe.
                # Exact source vertices determine support, not a guessed ankle.
                shoe=bpy.data.objects.get('SRC_Foot_'+side.upper())
                if shoe is None:
                    shoe=next(o for o in scene.objects if o.type=='MESH' and o.get('component_id')=='foot_'+side)
                minimum=min((rotation@(v.co-rest['foot_'+side].translation)).z for v in shoe.data.vertices)
                foot.z-=minimum
                posed['foot_'+side]=tr(foot)@rotation@rest['foot_'+side].to_3x3().to_4x4()
        ...
        old.name=old.name.replace('_v017','_v018')
    ...
    scene['model_source']='../model/chr_bunny01_model_v018.blend'
    scene.frame_set(1)
...
shutil.copy2(BASE/'v017/source/model/chr_bunny01_model_v017.blend',MODEL)
for lib in bpy.data.libraries:
    if 'chr_bunny01_model' in lib.filepath: lib.filepath='//../model/chr_bunny01_model_v018.blend'
...
report={'skeleton_sha256':sig,'model_unchanged':hashlib.sha256(MODEL.read_bytes()).hexdigest()==hashlib.sha256((BASE/'v017/source/model/chr_bunny01_model_v017.blend').read_bytes()).hexdigest(),
...
for scene in list(bpy.data.scenes):
    ...
    for view,position in [('threequarter',(2.8,4,1.8)),('side',(4,0,.9)),('front',(0,4,.74))]:
        if view=='front' and state not in ['moving','armed_moving']: continue
...
ledger={'asset_id':'CHR-PLY-CAPSULE01-3D-BUNNY01','version':'v018','status':'authored_pending_export','runtime_active_version':'v011',
...
(PACKAGE/'character_transfer_ledger_v018.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2))
print('V018_LIVELIER_JOG_VALIDATED',TARGET)
