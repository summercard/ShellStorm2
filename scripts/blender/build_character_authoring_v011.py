"""Create v011 rig and four authored cycles without changing mesh geometry.
...
def main():
    model = PACKAGE/'source/model/chr_bunny01_model_v011.blend'
    animation = PACKAGE/'source/animation/chr_bunny01_animation_v011.blend'
    if (model.exists() or animation.exists()) and '--replace-generated' not in sys.argv:
    ...
    bone('waist', (0,.000635,.17), (0,.000635,.32), 'root')
    bone('chest', (0,.000635,.32), (0,.000635,.47), 'waist')
    for name in ['head','ear_l','ear_r']:
        p = old_rest[name].translation
        tail = Vector((p.x,p.y,.99)) if name=='head' else Vector(((-1 if name=='ear_l' else 1)*.34,.015,1.46))
        bone(name, p, tail, 'chest' if name=='head' else 'head')
    for side, sign in [('l',-1),('r',1)]:
        wrist = old_rest['hand_'+side].translation
        ankle = old_rest['foot_'+side].translation
        ball = bpy.data.objects['SRC_Hand_Main_'+side.upper()]
        ball_center = sum((Vector(c) for c in ball.bound_box), Vector()) / 8
        direction = (ball_center-wrist).normalized()
        shoulder = wrist-direction*((abs(wrist.x)-.17)/abs(direction.x))
        elbow = shoulder.lerp(wrist,.5)
        bone('upper_arm_'+side, shoulder, elbow, 'chest')
        bone('forearm_'+side, elbow, wrist, 'upper_arm_'+side)
        bone('hand_'+side, wrist, ball_center, 'forearm_'+side)
        ...
        for name, x, length in [('thumb',-sign*.035,.035),('index',-sign*.012,.048),('fingers',sign*.022,.045)]:
            p = ball_center + Vector((0,x*.5,0))
            middle = p + direction*length*.55
            bone(name+'_01_'+side,p,middle,'hand_'+side)
            bone(name+'_02_'+side,middle,middle+direction*length*.4,name+'_01_'+side)
        ankle = Vector((ankle.x,ankle.y,.13))
        hip = Vector((ankle.x,ankle.y,.26))
        knee = hip.lerp(ankle,.5)
        bone('thigh_'+side,hip,knee,'waist')
        bone('shin_'+side,knee,ankle,'thigh_'+side)
        bone('foot_'+side,ankle,ankle+Vector((0,.11,-.045)),'shin_'+side)
    bpy.ops.object.mode_set(mode='OBJECT')
    rig.data.name = 'SKEL_bunny01_v003'
    rig['skeleton_id'] = 'SKEL-BUNNY01-003'
    ...
    scene['godot_forward'] = '-Z'
    scene['deformation_policy'] = 'rigid_components_unit_scale'
    ...
    scene['authoring_forward']='+Y'
    scene['godot_forward']='-Z'
    scene['model_source']='../model/chr_bunny01_model_v011.blend'
    scene['runtime_component_rest']=json.dumps({k:[list(row) for row in m] for k,m in old_rest.items()})
    ...
    def pose_cycle(state, t):
        ...
        waist_delta=transform((.012*sway if run else .008*math.sin(w),
                               .006 if armed else 0,
                               (.012+(.016 if not armed else .012)*bounce) if run else .010*breath),
                              ((-.045 if armed else -.025) if run else (-.02 if armed else .008*breath),
        ...
        head_delta=transform((.008*math.sin(w-.3),.003 if armed else 0,
        ...
        for side,sign in [('l',-1),('r',1)]:
            ...
            if run:
                ...
                if phase<.58:
                    ...
                    ease=u*u*(3-2*u)
                    foot.y+=-.075+.15*ease
                    lift=(.045 if armed else .06)*math.sin(math.pi*u)**2
                    pitch=.10*math.sin(TAU*u)
                foot.z+=lift
            ...
            if armed:
                wrist=Vector((sign*.19,.36 if side=='r' else .29,.25 if side=='r' else .32))
                wrist.z+=(.009*bounce if run else .005*breath)
                wrist.y+=.009*math.sin(w-.15)
                if run and side=='l': wrist.y+=.045*math.sin(w)
                hand_rot=(-.10+.02*breath,0,sign*1.445)
            else:
                wrist.y+=sign*.07*stride if run else .012*math.sin(w-.35)
                wrist.z+=(.015+.025*bounce) if run else .010*math.sin(w-.25)
                wrist.x+=sign*(.012+.012*math.sin(w-.2))
                hand_rot=(sign*.12*stride if run else .07*math.sin(w-.4),0,sign*.08)
            for upper,lower,end,target,origin,bend in [
                ...
                for n,p,q in [(u,start,joint),(l,joint,target)]:
                    ...
                    posed[n]=Matrix.LocRotScale(p,rotation,Vector((1,1,1)))
            ...
            posed[ear]=posed['head']@rest['head'].inverted()@rest[ear]@Euler(
                ((.055 if run else .025)*lag +(.09 if armed else 0),sign*.025*lag,sign*(.035+.03*lag))).to_matrix().to_4x4()
        return posed
    durations={'idle':3.2,'moving':.8,'armed_idle':2.8,'armed_moving':.8}
    for state,duration in durations.items():
        action=bpy.data.actions.new('anim_bunny01_'+state+'_v011')
        action.use_fake_user=True
        action['state_id']=state
        action['loop']=True
        action['duration']=duration
        action['authorship']='v011 original poses: breath / contact-down-pass-up / ready / stabilized armed stride'
    ...
    rig.animation_data.action=bpy.data.actions['anim_bunny01_idle_v011']
    scene.frame_start=1
    scene.frame_end=193
    scene.frame_set(1)
    bpy.context.view_layer.objects.active=rig
