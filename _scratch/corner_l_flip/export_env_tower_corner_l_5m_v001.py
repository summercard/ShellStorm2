...
Placement contract (must match the programmatic prefab it replaces):
    ...
    along Godot +X (long) and Godot -Z (short); the concave side (-X/-Z) is the
    room interior.  This is the contract `DungeonRoom3D._spawn_room_corner()`
    depends on -- it drops the module at the corner point and rotates it, so a
    recentred asset would shift every room corner.

    Godot +X arm  : X in [0, 5],   Z in [-0.15, 0.3175]  (armor on Godot +Z)
...
Why each transform is needed (measured, not assumed):
  ...
  * L placement is expressed in Blender space.  After `bake` the wall module
    ...
    -Y) and Z[0, 11.9] (height).  Godot (x, y, z) = Blender (x, z, -y):
        ...
        long arm  = Translation(+2.5, 0, 0)
            Blender X[-2.5,2.5] -> [0,5]; Y untouched -> Godot Z[-0.15,0.3175].
        short arm = Translation(0, +2.5, 0) @ Rotation(+90 deg, Blender Z)
            Rz(+90) sends local (x, y, z) -> (-y, x, z), so the 5m width lands on
            Blender Y (-> Godot -Z after the +2.5 shift: Y[0,5] -> Z[-5,0]) and
            the thickness lands on Blender X (local Y[-0.3175,0.15] -> X[-0.15,
            0.3175]); height is untouched.  The mirrored rotation (Rz(-90)) would
            flip the armor to Godot -X and break the outward-facing contract.
  ...
  * "Identical to two walls" is asserted, not assumed: each arm's own world AABB
    is recorded and compared against the v004 wall module's bounds, and the final
    triangle count must be exactly twice the wall's.
...
def placements():
    ...
    return [
        ("long", Matrix.Translation((ARM_SHIFT_M, 0.0, 0.0))),
        (
            "short",
...
def derive():
    ...
    # 180 deg yaw about Blender Z (so the decorated face lands on Godot +Z),
    # applied about the ROOT origin, then baked so the exported root is identity.
    ...
    merged["derivation"] = (
        ...
        "(long arm = T(+2.5,0,0); short arm = T(0,+2.5,0) @ Rz(+90)); each arm "
        "yawed 180 about Blender Z so decoration lands on Godot +Z / +X; ROOT "
        "offset baked into vertices and the empty dropped; four meshes joined "
        "into one; downward faces (world normal z < -0.5) deleted; triangulated."
    )
...
def verify_glb():
    ...
    # Decoration must face outward on BOTH arms: +Z (long arm) and +X (short arm).
    if highs[2] is not None and highs[2] <= 0.16:
        problems.append("long-arm decoration is not on Godot +Z (max Z = %s)" % highs[2])
