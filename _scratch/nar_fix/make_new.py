...
def write_crlf(rel, text):
    path = os.path.join(ROOT, rel)
    os.makedirs(os.path.dirname(path), exist_ok=True)
    # 行首 4 空格 → Tab（GDScript 用 Tab），只处理行首缩进，不动字符串内部空格。
    text = re.sub(r"(?m)^( +)", lambda m: "\t" * (len(m.group(1)) // 4), text)
    if rel.endswith(".gd"):
        if not any(line.startswith("\t") for line in text.split("\n")):
            raise SystemExit("缩进转换可能失败：%s" % rel)
        if re.search(r"(?m)^ +", text):
            raise SystemExit("仍有行首空格未转换：%s" % rel)
    out = text.replace("\n", "\r\n").encode("utf-8")
    open(path, "wb").write(out)
    print("wrote %-70s CR=%d LF=%d" % (rel, out.count(b"\r"), out.count(b"\n")))
...
[ext_resource type="Script" path="res://tests/verification/verify_opening_script_runtime.gd" id="1"]

[node name="VerifyOpeningScriptRuntime" type="Node3D"]
script = ExtResource("1")
'''

write_crlf("tests/verification/verify_opening_script_runtime.gd", GD)
write_crlf("tests/verification/verify_opening_script_runtime.tscn", TSCN)
