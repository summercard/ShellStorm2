...
func _ready() -> void:
	...
	if not base_avatar_scale.is_equal_approx(Vector3.ONE * Player3D.DEFAULT_BASE_SIZE_MULTIPLIER):
		failures.append("Player default 100% is not based on the new 80% authored size")
	if not is_equal_approx(float(player.get_debug_scale_snapshot().get("base_size_multiplier", 0.0)), 0.80):
		failures.append("Player debug snapshot does not expose the 80% base-size contract")
	...
	if failures.is_empty():
		print("PLAYER3D_DEBUG_SCALE_FLOW_OK: base=80% additive=100/110/120 collision_synced=true bounds=10..300 camera_unchanged=true")
		get_tree().quit(0)
		return
