...
func _verify_entry_mouse_aim(failures: Array[String]) -> void:
	...
	add_child(entry)
	await get_tree().process_frame
	_expect(entry.present(player), "开场页没有以纯视觉代理方案接管", failures)
	var snapshot := entry.get_entry_snapshot()
	_expect(bool(snapshot.get("uses_visual_snapshot", false)), "开场页不是视觉快照代理方案", failures)
	_expect(bool(snapshot.get("dedicated_camera", false)), "开场页没有建立独立展示摄像机", failures)
	_expect(bool(snapshot.get("dedicated_attributes", false)), "开场页展示摄像机没有独立attributes", failures)
	_expect(bool(snapshot.get("presentation_lights_only", false)), "开场页灯光没有限定为展示层专用", failures)
	var requested_yaw := player.aim_yaw + 0.73
	player.aim_yaw = requested_yaw
	entry.call("_process", 0.0)
	var snapshot_after := entry.get_entry_snapshot()
	_expect(not bool(snapshot_after.get("avatar_follows_mouse", true)), "开场页错误地声明跟随鼠标朝向", failures)
	_expect(bool(snapshot_after.get("presentation_facing_south", false)), "开场页角色没有固定面向南方", failures)
	_expect(bool(snapshot_after.get("camera_on_south_side", false)), "开场页摄像机没有固定在角色南侧", failures)
	_expect(not bool(snapshot_after.get("intro_spotlight_shadow_enabled", true)), "开场页展示灯不应产生展示外阴影", failures)
	_expect(is_equal_approx(player.aim_yaw, requested_yaw), "开场页错误地修改了真实玩家yaw", failures)
