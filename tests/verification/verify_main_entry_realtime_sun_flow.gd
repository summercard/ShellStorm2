...
func _verify_entry_mouse_aim(failures: Array[String]) -> void:
	...
	_expect(not bool(snapshot.get("avatar_follows_mouse", true)), "开场页错误地声明跟随鼠标朝向", failures)
	_expect(bool(snapshot.get("presentation_facing_south", false)), "开场页角色没有固定面向南方", failures)
	_expect(bool(snapshot.get("camera_on_south_side", false)), "开场页摄像机没有固定在角色南侧", failures)
	_expect(bool(snapshot.get("intro_spotlight_player_only", false)), "开场页角色补光没有隔离为玩家专用层", failures)
	_expect(not bool(snapshot.get("intro_spotlight_shadow_enabled", true)), "开场页角色补光不应产生展示外阴影", failures)
	_expect(not is_equal_approx(player.aim_yaw, requested_yaw), "开场页没有锁定南向展示朝向", failures)
