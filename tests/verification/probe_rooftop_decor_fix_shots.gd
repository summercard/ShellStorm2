...
## 五次修正（2026-09-21）追加 3 机位：⑦ 南/北墙贴墙掠视（绿化背贴外皮，背后不留地砖带）；
## ⑧ 原电灯开关所在西墙（改前实测挂在 (-29.66, 0.0, 6.7)，天台房间已不再生成该开关）。
...
const SHOTS := [
	# name, from, to
	# 南侧花圃 gz=+20.69（壳体南墙中心线 gz=20、外皮 20.15；花箱半进深 0.5925 ⇒ 背埋进外皮 0.05m）
	# ⇒ 机位必须站在花圃更南侧，压低高度看它**背贴墙皮**（2026-09-21 五次修正）。
	["01_ground_flowerbox_south", Vector3(-1.0, 1.6, 32.0), Vector3(-1.0, 0.8, 20.69)],
	# 东侧大盆栽 x=+15.97 / gz=−6.2（壳体东墙中心线 x=15、外皮 15.15；半进深 0.8727 ⇒ 背埋进外皮 0.05m）。
	["02_ground_plant_east", Vector3(24.5, 1.8, -6.2), Vector3(15.97, 1.1, -6.2)],
	# 东墙藤蔓 x=15.22，基座 gz∈{−8.5, 3.0, 11.0} + 上层叠高（顶高 7.4m）⇒ 拉开距离看整段竖高。
	...
	# —— 五次修正（2026-09-21）：绿化背贴墙 + 天台不再刷室内照明 ——
	# 南墙贴墙掠视：沿墙低角度横看，花箱背面应紧贴墙皮、背后不留可见地砖带（旧版空 1.4m）。
	["15_greenery_flush_south", Vector3(-21.0, 1.5, 24.0), Vector3(-2.0, 0.9, 20.7)],
	# 北墙贴墙掠视：同上，从东侧沿墙横看。
	["16_greenery_flush_north", Vector3(20.0, 1.5, -14.5), Vector3(2.0, 0.9, -10.7)],
	# 原电灯开关位置：改前实测挂在 (-29.66, 0.0, 6.7)（天台房间西墙，墙面 x=−30.0）⇒ 此面墙应已无开关。
	["17_no_light_switch_west", Vector3(-24.0, 2.4, 6.7), Vector3(-30.0, 1.2, 6.7)],
]
