...
func _verify_true_vision(dungeon: Dungeon3D, failures: Array[String]) -> void:
	...
	if flashlight == null:
		...
		if (
			...
			failures.append("Forward flashlight does not restore its configured real-light energy/range")
		if (
			spill == null
			...
			failures.append("Environment spill light does not restore its configured energy/range")
