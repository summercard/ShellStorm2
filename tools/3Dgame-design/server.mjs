...
async function writeBlockScenePayload(blockId, id, payload) {
  ...
  payload.blenderFile = `${sceneId}.blend`;
  const jsonPath = path.join(folder, 'scene.json'), blendPath = path.join(folder, `${sceneId}.blend`);
  const tempJsonPath = path.join(folder, '.scene.pending.json'), tempBlendPath = path.join(folder, `.${sceneId}.pending.blend`);
  await fs.writeFile(tempJsonPath, JSON.stringify(payload, null, 2), 'utf8');
  try {
    const blenderArgs = payload.blenderSource?.mode === 'bidirectional'
      ? ['--background', blendPath, '--python', blenderImportedUpdater, '--', tempJsonPath, tempBlendPath]
      : ['--background', '--factory-startup', '--python', blenderWriter, '--', tempJsonPath, tempBlendPath];
    await execFileAsync(blenderExecutable, blenderArgs, { maxBuffer: 10 * 1024 * 1024 });
    await fs.rename(tempBlendPath, blendPath);
    await fs.rename(tempJsonPath, jsonPath);
  } catch (error) {
    await Promise.allSettled([fs.unlink(tempJsonPath), fs.unlink(tempBlendPath)]);
    throw new Error(`Blender 文件生成失败：${error.stderr || error.message}`);
  }
  return payload;
}
...
async function api(req, res, url) {
  ...
  const genericImportMatch = url.pathname.match(/^\/api\/blocks\/([^/]+)\/import-blend$/);
  if (req.method === 'POST' && genericImportMatch) {
    try {
      const block = await requireProjectBlock(decodeURIComponent(genericImportMatch[1]));
      const sourceName = decodeURIComponent(String(req.headers['x-file-name'] || 'scene.blend'));
      if (!sourceName.toLowerCase().endsWith('.blend')) return json(res, 400, { error: '请选择 .blend 文件' });
      const sceneId = safeName(path.basename(sourceName, path.extname(sourceName))), folder = blockSceneFolder(block.id, sceneId);
      const blendPath = path.join(folder, `${sceneId}.blend`), pendingBlendPath = path.join(folder, '.uploaded.pending.blend');
      const jsonPath = path.join(folder, 'scene.json'), pendingJsonPath = path.join(folder, '.scene.importing.json');
      const modelDir = path.join(folder, 'models'), pendingModelDir = path.join(folder, '.models-importing');
      const chunks = []; let total = 0;
      for await (const chunk of req) { total += chunk.length; if (total > 512 * 1024 * 1024) throw new Error('Blender 文件超过512MB限制'); chunks.push(chunk); }
      if (!total) return json(res, 400, { error: 'Blender 文件为空' });
      await fs.mkdir(folder, { recursive: true }); await fs.writeFile(pendingBlendPath, Buffer.concat(chunks));
      await fs.rm(pendingModelDir, { recursive: true, force: true });
      await execFileAsync(blenderExecutable, ['--background', pendingBlendPath, '--python', blenderGenericImporter, '--', pendingJsonPath, pendingModelDir, sceneId, sourceName], { maxBuffer: 30 * 1024 * 1024 });
    ...
    } catch (error) { return json(res, 500, { error: `Blender 文件打开失败：${error.stderr || error.message}` }); }
  }
  ...
  const blockBlendMatch = url.pathname.match(/^\/api\/blocks\/([^/]+)\/scenes\/([^/]+)\/blend$/);
  if (blockBlendMatch && req.method === 'GET') {
    try {
      const blockId = decodeURIComponent(blockBlendMatch[1]), sceneId = safeName(decodeURIComponent(blockBlendMatch[2]));
      await requireProjectBlock(blockId);
      const blendPath = path.join(blockSceneFolder(blockId, sceneId), `${sceneId}.blend`), contents = await fs.readFile(blendPath);
      res.statusCode = 200; res.setHeader('Content-Type', 'application/x-blender');
      res.setHeader('Content-Disposition', `attachment; filename*=UTF-8''${encodeURIComponent(`${sceneId}.blend`)}`); res.end(contents); return true;
    } catch { return json(res, 404, { error: 'Blender 场景不存在，请先保存场景' }); }
  }
  ...
  if (blockSceneMatch && req.method === 'POST' && !blockSceneMatch[2]) {
    ...
    try {
      ...
      return json(res, 200, { id: name, blockId, name, updatedAt: payload.savedAt, blenderFile: payload.blenderFile });
