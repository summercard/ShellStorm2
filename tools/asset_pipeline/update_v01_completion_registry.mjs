...
// （敌人/场景/音效分属三个不同分账本），且本文件原先硬编码了另一台机器的 macOS 根目录。
...
// 本 payload 覆盖三个域：敌人 / 场景 / 音效。payload 内旧大类“音频”也必须在重放改造时迁移为“音效”。
...
if (!process.argv.includes("--i-know-the-rows-are-stale")) {
  throw new Error(
    "update_v01_completion_registry.mjs 的 rowNumber=226+i 是单体账本坐标，且 payload 跨敌人/场景/音效三个分账本。" +
