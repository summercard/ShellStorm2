...
## 红项是「必须恢复到初始值」的时序比较 → 先做「不动作对照组」（2026-09-24 实测）

这类断言形如 `absf(reopened - initial) > 0.025` / `x 必须恢复到初始值`。它红的时候**最容易被误判成
「我的改动让某个东西没恢复」**，但真凶常常是**场景自身的收敛过程**：两次采样时刻差了十几秒。

**判据**：红项涉及「两帧/两态相等」+ 两次采样时刻相差 >1 秒 ⇒ **先跑对照组，别读代码猜**。
对照组 = **把你正在验的那个动作整个去掉**，其余流程与采样点完全不变，只多等同样长的时间。

**实例（99F 关灯再开）**：`initial=0.3422 reopened=0.3155`（差 0.0267 > 阈值 0.025），看着像「有一盏灯没回来」。
> 对照组：**完全不碰开关**、只是静置 15 秒 ⇒ 亮度自己从 `0.3428` 掉到 **`0.3186`**，
> 与「重开后」的稳态值（0.3181~0.3185）**逐位相同** ⇒ 差异是「刚载入瞬间 vs 稳态」的时间漂移，与开关无关。
> （载入时大气/体积雾尚未收敛；`initial` 取的是载入瞬间那一帧。）

**修法**：让两帧都取自稳态（取 `initial` 前先等固定秒数），**不是**放宽阈值 ——
放大阈值只是把红挪到下一位小数，漂移方向一变又会红。

**配套：列表比状态差异，不要只看计数。** 计数相同不等于状态相同。把两侧的
「名字 + 路径 + 位置 + 能量 + 颜色」逐条打出来对比，一行 `PROBE_LIGHT <tag> #i path=... local=... energy=... color=...`
就能一次证伪「差的是灯」：
- 两列**逐位相同** ⇒ 灯没问题，去查渲染层/收敛/采样时刻；
- 两列**有出入** ⇒ 直接看到是哪一盏，不用再猜。

> 同理适用于 `verify_*_visual` 系列里一切「亮度/像素/节点数必须等于基线」的断言：先问
> **「这个量会不会自己随时间变？」**，会 ⇒ 采样时刻就是被测对象的一部分，必须固定住。
