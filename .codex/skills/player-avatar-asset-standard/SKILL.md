...
- 不安装本 Skill 到**项目本地**的 `.codex/skills`（`ShellStorm2/.codex/skills/`），除非用户以后明确授权；用户级 `~/.workbuddy/skills/` 与 `~/.codex/skills/` 是它的正式安装位置。
...
- 本 skill 涉及的分账本：角色（`ledgers/ShellStorm2_角色账本_v001.xlsx`）。
