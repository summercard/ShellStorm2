...
运行：
  python verify_packages_v006.py
"""
...
V006 = HERE.parent
BATTLE = V006.parent.parent.parent          # .../battle
ROOT = BATTLE.parents[4]
BASE = BATTLE / "components" / "entry_safe_room" / "v006"
SUMMARY = HERE / "export_packages_v006_summary.json"
...
check("24 package GLBs on disk", len(glbs) == 24, "found %d" % len(glbs))

for slug, meta in summary.items():
    path = BASE / slug / ("%s_visual_top3d_v006.glb" % slug)
    if not path.is_file():
