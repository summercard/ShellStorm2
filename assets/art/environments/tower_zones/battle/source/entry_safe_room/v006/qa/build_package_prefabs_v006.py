"""为入口安全房 v006 的 24 个房间自有资产包生成范式 B 的 runtime PackedScene。

范式 B（自包含可替换资产包）= 视觉 GLB + 稳定根节点 + 元数据，逐实例化：

  <slug>_root_top3d_v006.tscn
    ├─ Node3D 根（承载全部 metadata 契约与房间摆位）
    └─ ImportedModel  ← 实例化 <slug>_visual_top3d_v006.glb
...
PACKAGES_DIR = V006 / "component_packages_v006"
RUNTIME_DIR = BATTLE / "runtime" / "entry_safe_room" / "v006"
...
SOURCE_BLEND = f"{ROOM_ROOT}/source/entry_safe_room/{VERSION}/局内关卡01_入口安全房_15x15m_正式美术_{VERSION}.blend"
...
def main() -> int:
    summary = json.loads((HERE / "export_packages_v006_summary.json").read_text(encoding="utf8"))
    results = {}
    written = 0
    for category in CATEGORY_DIRS:
        ...
        for pkg_dir in sorted(cat_dir.iterdir()):
            ...
            if slug not in summary:
                raise SystemExit(f"{slug}: 不在 export_packages_v006_summary.json 里，先跑导出")
