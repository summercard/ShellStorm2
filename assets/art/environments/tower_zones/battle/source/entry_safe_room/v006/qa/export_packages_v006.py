"""导出入口安全房 v006 的 24 个房间自有资产包为 Godot 用 GLB（视觉，不含碰撞）。

范围
  - 只导出 catalog.json 列出的 24 个包（13 设施 + 5 墙面装饰 + 4 支持件 + 1 承重底板 + 1 地面装饰）。
...
背景模式的 matrix_world 陷阱
  ...
  再用与 qa/verify_entry_safe_room_v006.py 相同的 W() 重建式交叉验证；不一致就直接失败，
  不让一个静默的错误坐标进入 GLB。
...
HERE = Path(__file__).resolve().parent            # .../v006/qa
V006 = HERE.parent                                # .../v006
SOURCE_DIR = V006.parent.parent                   # .../source
BATTLE = SOURCE_DIR.parent                        # .../battle
ROOT = BATTLE.parents[4]                          # .../ShellStorm2
assert (ROOT / "assets" / "art").is_dir(), f"ROOT 解析失败: {ROOT}"

BLEND = V006 / "局内关卡01_入口安全房_15x15m_正式美术_v006.blend"
CATALOG = V006 / "component_packages_v006" / "catalog.json"
PALETTE = ROOT / "assets/art/shared/palette/设施低亮多巴胺色盘_10x10_512.png"
COMPONENTS_DIR = BATTLE / "components" / "entry_safe_room" / "v006"
VERSION = "v006"
...
def W(o):
    """背景模式安全的世界矩阵（与 qa/verify_entry_safe_room_v006.py 一致）。"""
...
out_summary = HERE / "export_packages_v006_summary.json"
