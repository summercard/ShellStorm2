...
OUT=ROOT+'/assets/art/environments/tower_zones/battle/source/room_types/l_corridor/v003'
with open(OUT+'/room_type_manifest.json') as f: manifest=json.load(f)
with open(OUT+'/component_packages_v003/catalog.json') as f: catalog=json.load(f)
...
for e in catalog:
    ...
    if rec.get('shared_component_asset_id') not in ('ENV-BATTLE-COMMON-FLOOR-TILE-R01-C01','ENV-BATTLE-COMMON-FLOOR-TILE-R01-C02'):
...
for e in catalog:
    if '/facility/' not in e['path']:continue
    with open(OUT+'/'+e['path']+'/asset_manifest.json') as f:rec=json.load(f)
    col=bpy.data.collections[rec['collection']]
    obs=[o for o in col.objects if o.type=='MESH']
