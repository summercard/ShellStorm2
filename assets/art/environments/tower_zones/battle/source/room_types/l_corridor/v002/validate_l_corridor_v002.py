...
OUT=ROOT+'/assets/art/environments/tower_zones/battle/source/room_types/l_corridor/v002'
with open(OUT+'/room_type_manifest.json') as f: manifest=json.load(f)
with open(OUT+'/component_packages_v002/catalog.json') as f: catalog=json.load(f)
...
base=ROOT+'/assets/art/environments/tower_zones/battle/source/room_types/l_corridor/v001/component_packages_v001'
with open(base+'/catalog.json') as f:previous=json.load(f)
locked=[e for e in previous if not e['path'].endswith(tuple('/'+s for s in ('crate_02','crate_03','console_01','console_02')))]
for e in locked:
    slug=e['path'].split('/')[-1];category=e['path'].split('/')[-2]
    with open(base+'/'+category+'/'+slug+'/asset_manifest.json') as f:old=json.load(f)
    with open(OUT+'/component_packages_v002/'+category+'/'+slug+'/asset_manifest.json') as f:new=json.load(f)
    if old['objects']!=new['objects']:
        issues.append('locked_package_changed:'+slug)
report['locked_packages_compared']=len(locked)
report['scope_lock']='pass' if not any(s.startswith('locked_package_changed:') for s in issues) else 'fail'
report['issues']=issues[:100]
