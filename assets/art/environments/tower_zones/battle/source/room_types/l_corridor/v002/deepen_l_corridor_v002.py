...
assert os.path.realpath(bpy.data.filepath)==os.path.realpath(V1+'/l_corridor_room_type_v001.blend'), bpy.data.filepath
os.makedirs(OUT,exist_ok=True)
for name in ('reference.png','top.png'):
    shutil.copy2(V1+'/'+name,OUT+'/'+name)
shutil.copytree(V1+'/component_packages_v001',OUT+'/component_packages_v002',dirs_exist_ok=True)
with open(V1+'/component_packages_v001/catalog.json') as f: catalog=json.load(f)
...
def oldpack(slug):
    ...
    with open(OUT+'/component_packages_v002/'+rec['path'].split('/',1)[1]+'/asset_manifest.json') as f: d=json.load(f)
...
def remove_old(slug):
    ...
    shutil.rmtree(OUT+'/component_packages_v002/'+rec['path'].split('/',1)[1])
    catalog.remove(rec)
...
for entry in catalog:
    entry['path']=entry['path'].replace('component_packages_v001','component_packages_v002')
    path=OUT+'/'+entry['path']+'/asset_manifest.json'
    with open(path) as f:rec=json.load(f)
    col=bpy.data.collections[rec['collection']]
    rec['version']='v002';rec['source_blend']='l_corridor_room_type_v002.blend'
...
for slug,info in packages.items():
    d=OUT+'/component_packages_v002/'+info['category']+'/'+slug
    os.makedirs(d,exist_ok=True);obs=[o for o in info['collection'].objects if o.type=='MESH']
    rec={'package_id':'ENV-BATTLE-LCORRIDOR-'+slug.upper(),'slug':slug,'name_zh':info['title'],'category':info['category'],'version':'v002','source_blend':'l_corridor_room_type_v002.blend','collection':info['collection'].name,'objects':[{'name':o.name,'location_m':[round(v,4) for v in o.location],'dimensions_m':[round(v,4) for v in o.dimensions]} for o in obs],'material_roles':[m.name for m in M],'exported':False,'collision':'pending Godot assembly'}
...
with open(OUT+'/component_packages_v002/catalog.json','w') as f:json.dump(catalog,f,ensure_ascii=False,indent=2)
with open(OUT+'/component_packages_v002/tree.txt','w') as f:f.write('\n'.join(e['path'] for e in catalog)+'\n')
room['package_count']=len(catalog);room['reference_image']='reference.png'
room['version']='v002';room['source_blend']='l_corridor_room_type_v002.blend'
...
bpy.ops.wm.save_as_mainfile(filepath=OUT+'/l_corridor_room_type_v002.blend')
