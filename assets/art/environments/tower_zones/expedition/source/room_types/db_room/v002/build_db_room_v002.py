"""数据库房间种类（不规则 40x30m · db_01 布局）美术源 v002。按用户指定参考图返工，Blender 4.5。

冻结契约：40x30m 包围盒 / 墙高 11.9m / 5m 模数 / 门洞 2.2x2.5 底 0.3m 过梁底 2.8m。
轮廓（db_01，非矩形）：主体 40x25 + 南侧中段外凸 10x5（面积 1050 m²）。

🔴 本房型的墙件继续 Library Link 引用 office_room/v001；地砖在 v002 改为受控派生：从 office 地砖组件复制网格并按几何/PaletteUV 删除重复白色斜小方块，保留其它结构、拼缝、磨损和 UV。office 原源不修改。
本轮新增/重排数据库机房专属设施：后左连续机柜列+服务推车、后右连续 L 工作台+三屏/工具挂板、中央双面维修工作岛、右前抬升检修平台，以及归组货架/服务终端/缆盘附区。
四材质、公共外链色盘、PaletteUV；每末级语义包一 Collection + asset_manifest.json。
公共唯一色盘 + 四共享材质角色 + PaletteUV；每末级语义包一 Collection + asset_manifest.json。
"""
from pathlib import Path
import bpy, bmesh, math, json, hashlib, random, sys, argparse
...
PKGDIR = OUT/'component_packages_v002'
...
REF = Path('C:/Users/zhuangmenghong/.workbuddy/clipboard-images/clipboard-2026-09-26T00-27-59-762Z-dddf2da5.jpg')
BLEND = '数据库房间种类_数据机房_40x30m_v002.blend'
...
root = collection('数据库房间_数据机房_v002', scene.collection)
...
cats = {k: collection(v, game) for k, v in [('floor', '01_地面系统'), ('facilities', '02_区域固定设施'),
                                            ('support', '03_环境支持')]}
link_host = collection('05_链接引用_办公室墙_v001_地砖受控派生_v002', root)
...
for i, (name, c) in enumerate(zip(roles, conf)):
    ...
    if i == 3:
        l.new(t.outputs['Color'], bs.inputs['Emission Color']); bs.inputs['Emission Strength'].default_value = 1.4
...
tile_source_names = {'地砖_%d_%d_资产包' % (ox, oy) for _ix, _iy, ox, oy in TILE_PLAN}
...
# 地砖受控派生：只从 office 输出包读取本地副本，按面中心 UV=白色色格且面积很小定位并删除白色斜小方块。
# office 原源和墙体链接保持只读；其余地砖拼缝、检修口、磨损、PaletteUV 和材质角色保留。
with bpy.data.libraries.load(str(OFFICE_BLEND), link=False) as (lib_from, lib_to):
    lib_to.collections = [n for n in lib_from.collections if n in tile_source_names]
TILE_SOURCE = {c.name: c for c in bpy.data.collections if c.name in tile_source_names and c.library is None}
missing_tiles = sorted(tile_source_names - set(TILE_SOURCE))
if missing_tiles:
    raise SystemExit('MISSING TILE DERIVATION SOURCES: %s' % missing_tiles)
TILE_VARIANTS = {}
tile_derivation_audit = []
LOCKED_PATHS = {str(p): hashlib.sha256(p.read_bytes()).hexdigest()
                for p in (OFFICE_BLEND, WHITEBOX, PALETTE, OUT.parent/'v001'/'build_db_room_v001.py')}

def face_signature(mesh, polygon):
    uv = mesh.uv_layers['PaletteUV']
    return tuple((tuple(mesh.vertices[mesh.loops[li].vertex_index].co), tuple(uv.data[li].uv))
                 for li in polygon.loop_indices) + (polygon.material_index,)

def derive_tile_mesh(source_obj, source_slug):
    me = source_obj.data.copy(); me.name = source_obj.data.name + '_db_v002_no_white_diagonal'
    uv = me.uv_layers.get('PaletteUV')
    if uv is None:
        raise AssertionError('tile missing PaletteUV: %s' % source_obj.name)
    bm = bmesh.new(); bm.from_mesh(me); uv_lay = bm.loops.layers.uv.get('PaletteUV')
    targets = set()
    for f in bm.faces:
        vals = [l[uv_lay].uv.copy() for l in f.loops] if uv_lay else []
        if not vals: continue
        u = sum(v.x for v in vals) / len(vals); v = sum(v.y for v in vals) / len(vals)
        if abs(u-.95) < .028 and abs(v-.05) < .028 and f.calc_area() < .30 and f.normal.z > .55:
            targets.add(f)
    # 斜块是独立盒体，沿共享边扩展整块连通面，避免留下白色小方块的侧面。
    remove = set(targets); frontier = list(targets)
    while frontier:
        f = frontier.pop()
        for e in f.edges:
            for nf in e.link_faces:
                if nf not in remove:
                    remove.add(nf); frontier.append(nf)
    bm.faces.index_update()
    removed_indices = {f.index for f in remove}
    expected = Counter(face_signature(me, p) for p in me.polygons if p.index not in removed_indices)
    removed = len(remove)
    bmesh.ops.delete(bm, geom=list(remove), context='FACES')
    bm.to_mesh(me); bm.free(); me.update()
    actual = Counter(face_signature(me, p) for p in me.polygons)
    assert expected == actual, '地砖未授权面几何、UV或材质索引发生变化'
    tile_derivation_audit.append({'source_package': source_slug, 'source_object': source_obj.name,
                                 'source_faces': len(source_obj.data.polygons), 'removed_faces': removed,
                                 'retained_faces': len(me.polygons), 'retained_geometry_uv_material_exact': expected == actual})
    return me, removed

def remap_tile_materials(mesh, source_obj):
    """将 append 得到的材质槽映射到本房 v002 四个共享角色，同时保留 polygon.material_index。"""
    source_slots = list(mesh.materials)
    is_emission = source_obj.name.startswith('自发光_') or len(source_slots) == 1 and any(
        (m and m.name.split('.')[0] == roles[3]) for m in source_slots)
    wanted = [roles[3]] if is_emission else roles[:3]
    old_indices = [p.material_index for p in mesh.polygons]
    mesh.materials.clear()
    for role in wanted:
        mesh.materials.append(mats[roles.index(role)])
    for p, old_index in zip(mesh.polygons, old_indices):
        if is_emission:
            p.material_index = 0
        else:
            old_mat = source_slots[old_index] if old_index < len(source_slots) else None
            old_name = old_mat.name.split('.')[0] if old_mat else roles[1]
            p.material_index = roles.index(old_name) if old_name in roles[:3] else min(old_index, 2)

def tile_variant_object(source_obj, source_slug, instance_tag, dst):
    key = (source_slug, source_obj.name)
    if key not in TILE_VARIANTS:
        TILE_VARIANTS[key] = derive_tile_mesh(source_obj, source_slug)
    me, removed = TILE_VARIANTS[key]
    o = bpy.data.objects.new('DERIVED_%s_%s' % (instance_tag, source_obj.name), me.copy())
    o['tile_source_object'] = source_obj.name
    o['tile_source_mesh'] = source_obj.data.name
    o['tile_source_package'] = source_slug
    o['provenance'] = 'controlled_derived_tile_v002'
    o['removed_white_diagonal_face_count'] = removed
    o['office_source_read_only'] = True
    remap_tile_materials(o.data, source_obj)
    o['material_role'] = 3 if len(o.data.materials) == 1 and o.data.materials[0] == mats[3] else 1
    o['palette_cell'] = [0, 0]
    current['col'].objects.link(o); current['items'].append(o)
    # append 的对象顶点已经保留 office 组件自身局部坐标，目标是组件根原点，不再减 matrix_basis。
    o.location = Vector(dst)
    o.rotation_euler = source_obj.rotation_euler
    o['source_room_type'] = 'office_room/v001'
    o['source_collection'] = source_slug
    o['provenance'] = 'controlled_derived_tile_v002'
    o['removed_white_diagonal_face_count'] = removed
    o['office_source_read_only'] = True
    return o
...
for ix, iy, ox, oy in TILE_PLAN:
    ...
    source_coll = TILE_SOURCE[cname]
    pack('derived_tile_%d_%d' % (ix, iy), '受控派生地砖_%d_%d_v002' % (ix, iy), 'floor', coll='visual_only', visual_only=True)
    dst = Vector((ix*GRID+GRID/2, iy*GRID+GRID/2, -0.32))
    derived = []
    for source_obj in source_coll.objects:
        if source_obj.type != 'MESH': continue
        derived.append(tile_variant_object(source_obj, cname, 'TILE_%d_%d' % (ix, iy), dst))
    tile_instances.append(dict(grid=[ix, iy], slug='tile_%d_%d' % (ox, oy), coll=cname,
                               ...
                               provenance='controlled_derived_tile_v002',
                               removed_white_diagonal_faces=sum(int(o.get('removed_white_diagonal_face_count', 0)) for o in derived)))

# 派生完成后卸载 append 的 office 地砖制作集合；输出仅保留 v002 派生网格，office 原源未写回。
for _c in list(TILE_SOURCE.values()):
    for _o in list(_c.objects):
        _data = _o.data
        bpy.data.objects.remove(_o, do_unlink=True)
        if _data and _data.users == 0: bpy.data.meshes.remove(_data)
    bpy.data.collections.remove(_c)
# TILE_VARIANTS 中的派生模板只用于复制，不能作为场景孤儿数据留在最终源文件。
for _me, _removed in list(TILE_VARIANTS.values()):
    if _me.users == 0:
        bpy.data.meshes.remove(_me)
for _me in list(bpy.data.meshes):
    if _me.library is None and _me.users == 0:
        bpy.data.meshes.remove(_me)
for _m in list(bpy.data.materials):
    if _m.library is None and _m.name not in roles and _m.users == 0:
...
# v002 去除旧版重复白色斜小方块；门区与作业区只保留结构拼缝、编号和受控警示边。
# 菱形警示标（黄）
...
# ================================================================ v002 组团重排与新增设施
# 保留 v001 的参考相机与白盒坐标；仅重排本房自持设施，墙件/门位/办公室源不动。
def package_center(p):
    # 这里的设施对象均无父级；matrix_basis 不依赖 depsgraph，避免刚创建对象的 matrix_world 尚未刷新。
    pts = [o.matrix_basis @ Vector(v) for o in p['items'] for v in o.bound_box]
    return Vector(((min(v.x for v in pts)+max(v.x for v in pts))/2,
                   (min(v.y for v in pts)+max(v.y for v in pts))/2, 0.0)) if pts else Vector((0, 0, 0))

def move_package(slug, target_xy):
    p = next((p for p in packages if p['slug'] == slug), None)
    if not p or not p['items']: return
    c = package_center(p); d = Vector((target_xy[0], target_xy[1], 0.0)) - c
    for o in p['items']: o.location += d
    p['layout_revision'] = 'v002_grouped_layout'
    p['layout_target_xy'] = list(map(float, target_xy))

# 参考图的组团关系：后左机柜列、后右 L 台、中央双面岛、右前抬升平台。
move_package('workbench_west', (-17.3, 3.0))
move_package('shelving_heavy_north', (-15.2, -6.0))
move_package('equipment_rack_ne', (16.7, 0.8))
# 散落旧组由下方语义独立的设施接替；旧中央单面台和旧机柜组同时撤下，避免重叠。
for _slug in ('crate_stack_field', 'planter_white_pair', 'console_tripod', 'tool_cart_yellow',
              'server_rack_row', 'rack_cluster_center', 'floor_service_pit', 'workbench_l_east',
              'center_worktable', 'tool_chest_red_pair', 'cable_reel_floor', 'bench_annex_southwest',
              'parts_drawer_cabinet', 'floor_zone_mark', 'floor_grate_grid'):
    _p = next((p for p in packages if p['slug'] == _slug), None)
    if _p:
        for _o in list(_p['items']):
            _data = _o.data if _o.type == 'MESH' else None
            if _o.name in _p['col'].objects: _p['col'].objects.unlink(_o)
            bpy.data.objects.remove(_o, do_unlink=True)
            if _data and _data.users == 0:
                bpy.data.meshes.remove(_data)
        _p['items'].clear(); _p['v001_scattered_group_removed'] = True
        packages.remove(_p)
        bpy.data.collections.remove(_p['col'])

# 后左连续柜列：每柜一个可替换包，正面朝 -Y，与参考图南侧观察方向一致。
for i, xx in enumerate((-16.5, -14.3, -12.1, -9.9, -7.7, -5.5)):
    pack('rear_server_%02d' % i, '后左服务器柜_%02d' % i, 'facilities')
    server_rack('服务器柜_%02d' % i, (0, 0, 0), rot=math.pi, side_light=False)
    for o in current['items']:
        o.location = Vector((xx, 11.5, 0)) + o.location * 1.45
        o.scale *= 1.45
pack('rear_server_bus', '后左机柜公共理线槽', 'support')
box('柜列底部线槽', (-11.0, 11.6, .15), (13.5, 1.60, .26), 0, DARK)
box('柜列顶部汇流槽', (-11.0, 11.7, 4.10), (13.5, .40, .22), 0, EDGE)

# 后右连续 L 台：退离北墙门槽，三屏、抽屉、挂板都附着于同一设施。
pack('workbench_l_east', '后右连续L形维修台', 'facilities')
box('长台桌面', (10.4, 10.6, 1.42), (13.8, 1.7, .16), 1, MID)
box('转角台桌面', (16.45, 7.05, 1.42), (1.7, 5.4, .16), 1, MID)
for xx in (4.4, 6.8, 9.2, 11.6, 14.0, 16.4):
    box('长台下柜', (xx, 10.7, .72), (2.10, 1.4, 1.35), 1, PANEL)
    for z in (.28, .57, .86, 1.15):
        box('维修台抽屉', (xx, 9.98, z), (1.9, .05, .23), 1, HULL, .008)
        box('维修台拉手', (xx, 9.92, z+.05), (.70, .07, .045), 0, STEEL, .004)
for yy in (5.3, 7.7):
    box('转角下柜', (16.45, yy, .72), (1.4, 2.1, 1.35), 1, PANEL)
for i, xx in enumerate((5.6, 8.4, 11.2)):
    start = len(current['items'])
    screen('后右三屏_%d' % i, (0, 0, 0), math.pi, True, CYAN)
    for o in current['items'][start:]:
        o.location = Vector((xx, 11.05, 1.50)) + o.location * 2.1
        o.scale *= 2.1
    box('三屏键盘', (xx, 10.16, 1.53), (1.20, .4, .06), 1, DARK, .008)
for xx in (13.6, 15.7):
    box('台背工具挂板', (xx, 11.44, 2.58), (1.92, .12, 1.85), 1, HULL)
    for j in range(6):
        x = xx-.75+j*.30
        cyl('挂板检修工具', (x, 11.34, 2.08), (x, 11.34, 2.75+(j%2)*.3), .045, 0, STEEL)
        box('工具手柄', (x, 11.31, 2.17), (.12, .1, .25), 1, AMBER, .012)
box('台背照明', (14.65, 11.32, 3.57), (4.1, .12, .08), 3, CYAN, .008)

# 中央双面维修工作岛：两侧可同时作业，带背靠背机架、三屏、工具挂板和两辆服务车。
pack('center_double_sided_repair_island', '中央双面维修工作岛', 'facilities')
IX, IY = 0.0, -0.8
for side in (-1, 1):
    yy = IY + side*1.05
    box('双面岛台面', (IX, yy, .96), (6.80, .92, .10), 1, MID, .014)
    box('双面岛台前缘', (IX, yy-side*.48, .91), (6.80, .06, .12), 1, HULL, .008)
    box('双面岛背板', (IX, IY-side*.50, 1.46), (6.80, .08, .92), 0, VOID, .008)
    for xx in (-2.70, 0.0, 2.70):
        box('双面岛端立板', (IX+xx, yy, .46), (.10, .78, .92), 0, DARK, .008)
        box('双面岛下柜', (IX+xx, yy-side*.22, .42), (1.05, .48, .68), 1, PANEL, .010)
    for xx in (-1.95, 0.0, 1.95):
        screen('双面岛三屏_%s' % side, (IX+xx, yy-side*.28, 1.02), 0.0 if side > 0 else math.pi, True, CYAN)
        box('双面岛键盘', (IX+xx, yy+side*.25, 1.02), (.52, .18, .03), 0, DARK, .004)
    for xx in (-2.25, -1.50, 1.50, 2.25):
        box('双面岛工具挂钩', (IX+xx, IY-side*.56, 1.65), (.10, .06, .46), 0, STEEL, .003)
        box('双面岛工具挂板灯', (IX+xx, IY-side*.59, 2.02), (.34, .03, .04), 3, BLUE, .002)
island_package = current
for xx in (-4.15, 4.15):
    pack('island_cart_%s' % ('west' if xx < 0 else 'east'), '维修岛服务车_%s' % ('西' if xx < 0 else '东'), 'facilities')
    box('双面岛服务车箱', (IX+xx, IY, .55), (.72, 1.16, .95), 1, PANEL, .012)
    box('双面岛服务车顶盘', (IX+xx, IY, 1.05), (.82, 1.24, .06), 0, EDGE, .006)
    for z in (.28, .50, .72):
        box('双面岛服务车抽屉', (IX+xx, IY-.63, z), (.56, .03, .13), 1, [RED, AMBER, BLUE][int(z*10)%3], .003)
    casters('双面岛服务车脚轮', IX+xx, IY, 0.0, .28, .42, .045)
current = island_package
box('双面岛顶部工具桥', (IX, IY, 2.42), (6.40, .18, .18), 0, EDGE, .008)
for xx in (-2.4, -1.2, 0, 1.2, 2.4):
    cyl('双面岛顶部挂具', (IX+xx, IY, 2.48), (IX+xx, IY, 2.90), .035, 0, STEEL, 8)

# 后左连续机柜服务推车补强，形成参考图中的设备带而非孤立柜体。
for i, (xx, yy) in enumerate(((-16.5, 8.8), (-11.0, 8.8), (-5.5, 8.8))):
    pack('rear_service_cart_%02d' % i, '机柜服务推车_%02d' % i, 'facilities')
    box('机柜服务推车箱体', (xx, yy, .48), (1.18, .82, .86), 1, PANEL, .012)
    box('机柜服务推车顶盘', (xx, yy, .95), (1.28, .92, .06), 0, EDGE, .006)
    for z in (.25, .43, .61):
        box('服务推车抽屉', (xx, yy-.43, z), (.86, .03, .13), 1, [BLUE, AMBER, RED][i], .003)
    box('服务推车屏', (xx, yy-.46, 1.08), (.40, .03, .22), 3, CYAN, .002)
    casters('服务推车脚轮', xx, yy, 0, .42, .28, .045)
# 公共理线已经独立归入 rear_server_bus；不再生成悬空补强梁及重复的三屏挂板组。

# 右前附区：货架、服务终端、缆盘分别成包，避免把可替换设施焊成区域总包。
RX, RY = 15.2, -8.4
for i, xx in enumerate((RX-2.0, RX, RX+2.0)):
    pack('right_front_shelf_%02d' % i, '右前货架_%02d' % i, 'facilities')
    for z in (.20, 1.15, 2.10):
        box('附区货架层板', (xx, RY, z), (1.72, .72, .07), 1, PANEL, .008)
    for dx in (-.80, .80):
        for yy in (RY-.34, RY+.34):
            box('附区货架立柱', (xx+dx, yy, 1.25), (.08, .08, 2.50), 0, DARK, .006)
    for z, cell in ((.48, TAN), (1.43, CARD), (2.38, TEAL)):
        box('附区周转箱', (xx, RY, z), (1.28, .56, .42), 1, cell, .006)
pack('right_front_service_terminal', '右前服务终端', 'facilities')
box('附区服务终端柜', (RX, RY-1.10, .72), (1.20, .64, 1.44), 1, PANEL, .012)
box('附区服务终端面', (RX, RY-1.44, 1.02), (.82, .03, .50), 3, CYAN, .003)
box('附区终端键盘', (RX, RY-1.34, .70), (.58, .20, .03), 0, DARK, .003)
for i, xx in enumerate((RX+2.1, RX+3.1)):
    pack('right_front_cable_reel_%02d' % i, '右前服务缆盘_%02d' % i, 'facilities')
    box('附区缆盘底架', (xx, RY-1.0, .08), (1.04, .72, .16), 0, VOID, .008)
    cyl('附区缆盘法兰', (xx-.32, RY-1.0, .48), (xx-.25, RY-1.0, .48), .42, 1, [RED, BLUE][i], 20)
    cyl('附区缆盘法兰', (xx+.32, RY-1.0, .48), (xx+.25, RY-1.0, .48), .42, 1, [RED, BLUE][i], 20)
    cyl('附区缆盘芯筒', (xx-.26, RY-1.0, .48), (xx+.26, RY-1.0, .48), .24, 0, CARD, 16)
    casters('附区缆盘脚轮', xx, RY-1.0, 0, .30, .24, .045)

# 右前带门架的抬升检修平台：平台、液压柱、黄黑边框与跨梁形成清晰维修工位。
pack('raised_service_platform', '右前抬升检修平台', 'facilities')
SPX, SPY = 9.6, -4.6
box('抬升平台底座', (SPX, SPY, .08), (4.80, 3.55, .16), 0, VOID, .008)
box('抬升平台主体', (SPX, SPY, .34), (4.42, 3.18, .46), 1, DARK, .014)
box('抬升平台检修台面', (SPX, SPY, .63), (4.10, 2.86, .14), 1, PANEL, .008)
for px in (SPX-1.72, SPX+1.72):
    for py in (SPY-1.12, SPY+1.12):
        cyl('平台液压升降柱', (px, py, .64), (px, py, 1.32), .13, 0, STEEL, 12)
        box('平台柱脚', (px, py, .70), (.36, .36, .10), 0, EDGE, .006)
for side in (-1, 1):
    box('平台黄黑警示边', (SPX, SPY+side*1.49, .73), (4.32, .10, .08), 1, AMBER, .004)
    for j in range(8):
        box('平台边缘斜纹', (SPX-1.68+j*.48, SPY+side*1.49, .79), (.22, .12, .025), 0, VOID, .002, math.radians(35))
box('平台检修设备', (SPX, SPY, .90), (1.60, 1.20, .34), 1, HULL, .010)
box('平台设备控制面', (SPX, SPY-0.64, .96), (.70, .03, .18), 3, CYAN, .003)
pack('raised_service_gantry', '抬升平台检修门架', 'facilities')
for px in (SPX-1.86, SPX+1.86):
    box('门架立柱', (px, SPY+1.08, 2.55), (.18, .18, 3.90), 0, EDGE, .010)
    box('门架立柱护套', (px, SPY+1.08, 1.48), (.30, .30, 1.80), 1, PANEL, .008)
    box('门架立柱警示箍', (px, SPY+1.08, 2.15), (.34, .34, .10), 1, AMBER, .003)
box('门架顶部跨梁', (SPX, SPY+1.08, 4.40), (4.00, .22, .22), 0, EDGE, .010)
box('门架顶部滑轨', (SPX, SPY+.86, 4.18), (3.42, .12, .10), 0, DARK, .006)
box('门架吊挂检修灯', (SPX, SPY+.86, 3.82), (.62, .18, .16), 3, HI2, .004)
for px in (SPX-.70, SPX, SPX+.70):
    cyl('门架吊挂链', (px, SPY+.86, 3.98), (px, SPY+.86, 3.55), .028, 0, STEEL, 8)
...
for p in packages:
    ...
    for o in p['items']:
        ...
        for cx in (b[0][0], b[1][0]):
            ...
            break
oob = sorted(set(oob))
print('V002_OOB_PACKAGES', sorted({x.split(':', 1)[0] for x in oob}), flush=True)
for _slug in sorted({x.split(':', 1)[0] for x in oob}):
    _p = next(p for p in packages if p['slug'] == _slug)
    _bb = bounds(_p['items'])
    print('V002_OOB_BOUNDS', _slug, _bb, flush=True)
...
# 所有权检查：墙保持 Library Link，42 块地砖明确属于受控派生包，禁止无归属本地网格。
link_libs = {c.library.filepath for c in LINKED.values()}
authored_mesh_names = {o.data.name for p in packages for o in p['items']}
local_meshes = [m for m in bpy.data.meshes if m.library is None and m.name not in authored_mesh_names]
linked_meshes = sorted({o for c in LINKED.values() for o in c.all_objects if o.type == 'MESH'}, key=lambda o: o.name)
linked_materials = sorted({m for o in linked_meshes for m in o.data.materials if m}, key=lambda m: m.name)

def strict_uv_ok(obj):
    layer = obj.data.uv_layers.get('PaletteUV')
    if layer is None: return False
    for poly in obj.data.polygons:
        uvs = [layer.data[li].uv for li in poly.loop_indices]
        cols = {min(9, max(0, math.floor(max(0.0, min(.999999, uv.x))*10))) for uv in uvs}
        rows = {min(9, max(0, math.floor(max(0.0, min(.999999, uv.y))*10))) for uv in uvs}
        area = abs(sum(uvs[i].x*uvs[(i+1)%len(uvs)].y - uvs[(i+1)%len(uvs)].x*uvs[i].y
                       for i in range(len(uvs))))*.5 if len(uvs) >= 3 else 0.0
        if len(cols) != 1 or len(rows) != 1 or area < 1e-7:
            return False
        col, row = next(iter(cols)), next(iter(rows))
        if not all(col/10+.01 <= uv.x <= (col+1)/10-.01 and
                   row/10+.01 <= uv.y <= (row+1)/10-.01 for uv in uvs):
            return False
    return True

linked_uv_results = [{'object': o.name, 'mesh': o.data.name,
                      'palette_uv': bool(o.data.uv_layers.get('PaletteUV')),
                      'faces': len(o.data.polygons), 'strict_uv_pass': strict_uv_ok(o)}
                     for o in linked_meshes]
linked_dependency_audit = {
    'linked_collection_count': len(LINKED),
    'linked_mesh_object_count': len(linked_meshes),
    'linked_materials': [{'name': m.name, 'library': m.library.filepath if m.library else None,
                          'users': m.users} for m in linked_materials],
    'linked_mesh_uv_audit': linked_uv_results,
    'strict_uv_pass': all(r['strict_uv_pass'] for r in linked_uv_results),
    'linked_material_dependency_pass': all(m.users > 0 for m in linked_materials),
    'scope': 'task-level recursive dependency audit; generic validator --all-meshes does not traverse collection instances',
}
...
def merge_package(p, emission):
    ...
    dg = bpy.context.evaluated_depsgraph_get(); vs = []; fs = []; mi = []; uv_faces = []
    ...
    for o in objs:
        ...
        source_uv = me.uv_layers.get('PaletteUV')
        preserve_source_uv = p['category'] == 'floor' and p['slug'].startswith('derived_tile_')
        for f in me.polygons:
            ...
            if emission:
                ...
                role_name = None
                if f.material_index < len(me.materials) and me.materials[f.material_index]:
                    role_name = me.materials[f.material_index].name.split('.')[0]
                role = roles.index(role_name) if role_name in roles[:3] else int(o.get('material_role', 1))
                role = min(max(role, 0), 2)
            mi.append(used.index(role))
            if source_uv and preserve_source_uv:
                uv_faces.append([source_uv.data[li].uv.copy() for li in f.loop_indices])
            else:
                cell = list(o.get('palette_cell', [9, 1]))
                uv_faces.append([Vector(((cell[0]+.5)/10+.022*math.cos(2*math.pi*j/len(f.loop_indices)),
                                         1-(cell[1]+.5)/10+.022*math.sin(2*math.pi*j/len(f.loop_indices))))
                                 for j in range(len(f.loop_indices))])
        ev.to_mesh_clear()
    ...
    for poly, mat, face_uv in zip(mesh.polygons, mi, uv_faces):
        poly.material_index = mat
        for li, uv_value in zip(poly.loop_indices, face_uv):
            uv.data[li].uv = uv_value
    ...
    obj['version'] = 'v002'; obj['collision_owner'] = p['collision_owner']
...
for p in packages:
    ...
    rec = {'package_id': 'ENV-EXPEDITION-L01-DB-'+p['slug'].upper(), 'slug': p['slug'],
           'name_zh': p['title'], 'category': p['category'], 'version': 'v002', 'source_blend': BLEND,
           'collection': p['col'].name, 'objects': [o.name for o in merged], 'world_origin_m': origin,
           'local_origin': 'bottom_center', 'provenance': ('controlled_derived_tile_v002' if p['slug'].startswith('derived_tile_') else 'authored_here'),
...
# 链接引用清单：仅墙件属于 Library Link；地砖另列为受控派生来源追溯。
linked_records = []
for w in wall_instances:
    linked_records.append({'kind': 'wall_bay', 'source_package_collection': w['coll'],
                           ...
                           'is_door_wall': w['is_door'], 'provenance': 'library_link'})
derived_tile_records = []
for t in tile_instances:
    derived_tile_records.append({'kind': 'floor_tile_controlled_derived',
                                  'source_package_collection': t['coll'],
                                  ...
                                  'provenance': 'controlled_derived_tile_v002',
                                  'removed_white_diagonal_faces': t['removed_white_diagonal_faces']})
linked_summary = {}
...
manifest = {
    'schema': 'shellstorm2.room_type_art_manifest', 'schema_version': 1, 'version': 'v002',
    ...
    'controlled_derived_tile_variant': {
        'source_room_type': 'office_room/v001',
        'source_packages': sorted(tile_source_names),
        'removed_geometry': 'all small diagonal white floor-marker faces located by PaletteUV white cell and small upward-facing face area',
        'office_source_modified': False,
        'provenance': 'controlled_derived_tile_v002'
    },
    'linked_component_library': {
        'source_room_type': 'office_room/v001',
        'source_blend': str(OFFICE_BLEND.relative_to(ROOT)),
        'mechanism': '墙体使用 bpy.data.libraries.load(link=True) 集合实例；地砖使用 link=False 受控派生，'
                     '仅删除授权白色斜小方块并重映射到本房四材质；墙实例空物体 L = P_dst - R * origin_src',
        'wall_bays_linked': len(wall_instances), 'floor_tiles_linked': 0,
        'floor_tiles_controlled_derived': len(tile_instances),
    ...
    'required_components': ['linked office 5m plain wall bays (%d distinct source packages, %d instances)'
                            ...
                            'controlled-derived office 5m floor deck tiles (%d instances)' % len(tile_instances),
                            'center double-sided repair island', 'rear-left service cart bank',
                            'rear-right continuous L workbench with three screens and tool boards',
                            'right-front independent shelving, service terminal and cable reels',
                            'raised service platform and gantry'],
    'required_facilities': ['rear-left six-server cabinet bank', 'rear-left three service carts',
                            'rear-right continuous L-shaped repair workbench', 'rear-right three screens and tool boards',
                            'centre double-sided repair island with two service carts',
                            'right-front three independent shelving units', 'right-front service terminal',
                            'right-front two independent cable reels', 'raised service platform',
                            'raised service gantry', 'wall light pylons and service runs',
                            'ceiling cable trays, pipes, ducts and lights'],
    ...
    'linked_instance_count': len(wall_instances),
    'controlled_derived_instance_count': len(tile_instances),
...
qa = {'status': 'CONDITIONAL_PASS' if all(checks.values()) else 'FAIL',
      'task_level_status': 'PASS' if all(checks.values()) else 'FAIL',
      'strict_validator_status': 'FAIL_NO_UNUSED_MATERIALS_SCOPE_LIMITATION',
      'strict_validator_note': 'validate_game_prop.py --all-meshes only walks scene.objects; linked collection-instance meshes/materials are audited separately in linked_dependency_audit and must not be deleted',
      'checks': checks,
      ...
      'scope_lock': {'whitebox_sha256': hashlib.sha256(WHITEBOX.read_bytes()).hexdigest(),
                     ...
                     'method': 'all new art geometry authorized for authoring; office wall geometry remains Library Link; '
                               'floor tile geometry is controlled-derived with only authorized white diagonal faces removed; '
                               'preserve whitebox, office source and reference by hash',
                     'locked_input_sha256': LOCKED_PATHS},
      'visual_review': {'status': 'reviewed_latest_render_conditional',
                        'views': ['reference_match_isometric', 'top_plan', 'cutaway_sw', 'west_rack_row',
                                  'east_workbench', 'centre_workshop', 'tab_entry'],
                        'reference': str(REF),
                        'fixed_camera_setup': 'reference-match camera plus six retained v001 comparison cameras',
                        'deepening': ['rear-left continuous six-rack bank with three service carts',
                                      'rear-right continuous L repair bench with three screens and tool boards',
                                      'compact double-sided central repair island with two service carts',
                                      'raised service platform with gantry in the right-front annex',
                                      'independent shelves, terminal and cable reels',
                                      'controlled-derived tiles remove only white diagonal marker geometry']},
...
for name, data in [('room_type_manifest.json', manifest), ('component_inventory.json', records),
                   ('linked_office_components.json',
                    {'schema': 'shellstorm2.room_type_linked_components', 'schema_version': 1,
                     ...
                     'controlled_derived_instances': len(derived_tile_records),
                     'packages': [{'kind': k, 'linked_slug': s, 'instance_count': n}
                                  for (k, s), n in sorted(linked_summary.items())],
                     'instances_detail': linked_records,
                     'controlled_derived_tile_instances': derived_tile_records}), ('qa_report.json', qa)]:
...
cam_match = camera('参考镜头_用户参考匹配', (31, -34, 38), (0, 0, 0.6), 48)
cam_match['reference_image'] = str(REF)
cam_match['reference_sha256'] = hashlib.sha256(REF.read_bytes()).hexdigest()
cam_match['comparison_role'] = 'primary_reference_match'
...
if not opts.no_render:
    # 用户参考匹配主镜头：先固定构图，再输出旧六镜头对照。
    render(cam_match, 'db_00_reference_match', {'west', 'east', 'south_w', 'south_e', 'tab_w', 'tab_e'}, True, True)
...
scene.camera = cam_match
scene['reference_camera'] = cam_match.name
scene['reference_image'] = str(REF)
scene['reference_sha256'] = hashlib.sha256(REF.read_bytes()).hexdigest()
qa['linked_dependency_audit'] = linked_dependency_audit
qa['tile_derivation_audit'] = tile_derivation_audit
qa['saved_display_objects'] = {'camera_count': len([o for o in display.objects if o.type == 'CAMERA']),
                               'light_count': len([o for o in display.objects if o.type == 'LIGHT']),
