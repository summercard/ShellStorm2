...
OUT=ROOT+'/assets/art/environments/tower_zones/expedition/source/room_types/l_corridor/v003'
with open(OUT+'/room_type_manifest.json') as f: manifest=json.load(f)
with open(OUT+'/component_packages/catalog.json') as f: catalog=json.load(f)
...
if len([x for x in catalog if '/floor/' in x['path']])!=42: issues.append('tile_count')
...
expected_tiles={(x,y) for x in range(9) for y in range(3)} | {(x,y) for x in (6,7,8) for y in range(-5,0)}
actual_tiles=set()
for e in catalog:
    if '/floor/' not in e['path']:continue
    with open(OUT+'/'+e['path']+'/asset_manifest.json') as f:rec=json.load(f)
    col=bpy.data.collections[rec['collection']]
    bases=[o for o in col.objects if o.name.startswith('5米结构地砖')]
    shared=[o for o in col.objects if o.name.startswith('通用地砖表层')]
    if len(bases)!=1 or len(shared)!=2:issues.append('shared_tile_structure:'+e['path']);continue
    x=round((bases[0].location.x-2.5)/5);y=round((bases[0].location.y-2.5)/5)
    actual_tiles.add((x,y))
    if rec.get('shared_component_asset_id') not in ('ENV-SHARED-GENERIC-FLOOR-TILE-R01-C01','ENV-SHARED-GENERIC-FLOOR-TILE-R01-C02'):
        issues.append('shared_tile_id:'+e['path'])
if actual_tiles!=expected_tiles:issues.append('footprint_tiles')
if manifest['dimensions_m']!={'outer_bounds':[45,40],'horizontal_arm':[45,15],'south_arm_extension':[15,25]}:
    issues.append('dimensions_contract')
if len(manifest.get('reserved_door_slots',[]))!=4 or manifest.get('door_geometry_in_source') is not False:
    issues.append('door_slot_contract')
for e in catalog:
    if '/facilities/' not in e['path']:continue
    with open(OUT+'/'+e['path']+'/asset_manifest.json') as f:rec=json.load(f)
    col=bpy.data.collections[rec['collection']]
    obs=[o for o in col.objects if o.type=='MESH']
    cx=sum(o.location.x for o in obs)/len(obs);cy=sum(o.location.y for o in obs)/len(obs)
    distance=min(15-cy,cy) if cy>=0 and cx<30 else (min(15-cy,45-cx) if cy>=0 else min(cx-30,45-cx))
    if distance>2.8:issues.append('facility_off_wall:'+e['path'])
wall_centers=[]
for e in catalog:
    ...
    for o in col.objects:
        if o.name.startswith('标准5米墙体') and o.dimensions.z>10:
            wall_centers.append((o.location.x,o.location.y))
for x,y in manifest['door_centers_m']:
    if any(abs(a-x)<.2 and abs(b-y)<.2 for a,b in wall_centers):
        issues.append('door_slot_blocked:'+str((x,y)))
guide_count=len(bpy.data.collections['01_制作组件_按设施拆分'].objects)
if guide_count!=42:issues.append('guide_count')
report={'status':'pass' if not issues else 'fail','source_blend':bpy.data.filepath,'mesh_objects':len(meshes),'packages':len(catalog),'floor_tile_packages':42,'faces_total':face_count,'faces_valid_palette_uv':good_faces,'materials':len(bpy.data.materials),'wall_visual_height_m':11.9,'wall_logic_height_m':12,'door_centers_m':manifest['door_centers_m'],'scope_lock':'new source, no previous asset to compare','issues':issues[:100]}
report['corridor_width_m']=15
report['door_slots_reserved']=4
report['shared_tile_packages']=len(actual_tiles)
report['authoring_grid_guides']=guide_count
