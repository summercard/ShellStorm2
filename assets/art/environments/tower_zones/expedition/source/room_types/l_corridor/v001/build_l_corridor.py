...
OUT = ROOT + '/assets/art/environments/tower_zones/expedition/source/room_types/l_corridor/v001'
...
bpy.ops.wm.save_as_mainfile(filepath=OUT+'/L型走廊种类_数据连廊_45x35m_v001.blend')

catalog=[]
for slug, info in packages.items():
    category=info['category']; d=OUT+'/component_packages/'+category+'/'+slug
    ...
    rec={'package_id':'ENV-EXPEDITION-L-CORRIDOR-'+slug.upper(), 'slug':slug,'name_zh':info['name_zh'],'category':category,'version':'v001','source_blend':'L型走廊种类_数据连廊_45x35m_v001.blend','collection':info['collection'].name,'objects':entries,'material_roles':MAT_NAMES,'exported':False,'collision':'pending Godot assembly'}
