...
# 造型共用件仍取自 battle 区块通用组件库 v003（真实存在的造型源）；运行时引用统一走
# tower_zones/shared/source/common_components/ 的 ENV-SHARED-GENERIC-* 资产。
COMMON=ROOT+'/assets/art/environments/tower_zones/battle/source/common_components/v003/战局区块_通用组件库_v003.blend'
assert os.path.realpath(bpy.data.filepath)==os.path.realpath(V2+'/L型走廊种类_数据连廊_45x35m_v002.blend'),bpy.data.filepath
os.makedirs(OUT,exist_ok=True)
for name in ('reference.png','reference_crop_a.png','reference_crop_b.png'):
    shutil.copy2(V2+'/renders/'+name,OUT+'/renders/'+name)
shutil.copytree(V2+'/component_packages',OUT+'/component_packages',dirs_exist_ok=True)
with open(V2+'/component_packages/catalog.json') as f: catalog=json.load(f)
...
def manifest_path(entry):
    return OUT+'/'+entry['path'].replace('component_packages','component_packages')+'/asset_manifest.json'
def pack(slug):
...
def clone_pack(template_slug,new_slug,dx,dy,category=None):
    ...
    catalog.append({'package_id':'ENV-EXPEDITION-L-CORRIDOR-'+new_slug.upper(),
                    'path':'component_packages/'+cat+'/'+new_slug,
...
for e in catalog:
    ...
    e['_shared_component_id']='ENV-SHARED-GENERIC-FLOOR-TILE-R01-C01' if meshes is tiles_A else 'ENV-SHARED-GENERIC-FLOOR-TILE-R01-C02'
...
for e in catalog:
    ...
    for ob in col.objects:
        ...
        e['_shared_component_id']='ENV-SHARED-GENERIC-WALL-STANDARD-5M'
...
for e in catalog:
    ...
    rec['version']='v003';rec['source_blend']='L型走廊种类_数据连廊_45x40m_v003.blend'
    ...
    e['path']=e['path'].replace('component_packages','component_packages')
...
room['version']='v003';room['source_blend']='L型走廊种类_数据连廊_45x40m_v003.blend'
...
room['shared_door_wall_component_id']='ENV-SHARED-GENERIC-WALL-DOOR-5M'
room['shared_door_component_id']='ENV-SHARED-GENERIC-DOOR-5M'
room['shared_standard_wall_component_id']='ENV-SHARED-GENERIC-WALL-STANDARD-5M'
room['shared_floor_tile_component_ids']=['ENV-SHARED-GENERIC-FLOOR-TILE-R01-C01','ENV-SHARED-GENERIC-FLOOR-TILE-R01-C02']
...
bpy.ops.wm.save_as_mainfile(filepath=OUT+'/L型走廊种类_数据连廊_45x40m_v003.blend')
