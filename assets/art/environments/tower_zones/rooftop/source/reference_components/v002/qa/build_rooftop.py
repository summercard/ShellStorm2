...
scene=bpy.context.scene;scene.name='天台组件库与参考拼装_v002'
...
outroot=coll('02_游戏输出_独立资产包_v002',root)
...
def door():
    ...
    # User revision: genuine masonry depth around the recessed door leaf.
    for x in [-2.17,2.17]:
        for j in range(4):
            box((x,-.22,.895+j*1.79),(.54,1.10,1.775),(9,7),1,'厚石材门柱',.045)
    box((0,-.22,7.28),(4.88,1.16,.56),(9,8),1,'厚门楣',.045)
    box((0,-.42,.08),(4.90,1.55,.16),(9,7),1,'进深门槛',.025)
    # Move the existing lamp onto the front of the lintel, rather than burying it.
    for name in ['门灯罩','自发光灯芯']:
        p=parts[name];p.v=[(x,y-.70,z) for x,y,z in p.v]

def attached_vine(x,y,z,w,h,rotation=0):
    global parts
    host=parts;parts={};vine(0,0,z,w,h);vegetation=parts;parts=host
    co,si=math.cos(rotation),math.sin(rotation)
    for name,p in vegetation.items():
        q=part(name);k=len(q.v)
        q.v.extend([(x+co*a-si*b,y+si*a+co*b,c) for a,b,c in p.v])
        q.f.extend([tuple(k+i for i in face) for face in p.f]);q.style.extend(p.style)

def ivy_corner(inner=False):
    parapet(angle=-1 if inner else 1)
    attached_vine(0,-1.30,1.82,2.35,1.65)
    attached_vine(1.30 if inner else -1.30,.1,1.82,2.2,1.65,math.pi/2 if inner else -math.pi/2)

def ivy_wall(kind='plain',external=False):
    wall(kind,external)
    if kind=='door':
        attached_vine(-1.35,-.18,11.83,2.1,4.25)
        attached_vine(1.55,-.18,11.83,1.4,3.9)
    elif external:
        attached_vine(0,-.18,11.83,4.7,5.4)
    else:
        attached_vine(-1.45,-.18,11.83,1.8,7.5)
        attached_vine(.8,-.18,11.83,2.3,3.5)
def canopy():
 ...
 *[(f'detail_{k}',n,'09_细节附件',lambda k=k:detail(k)) for k,n in [('mat','编织门垫'),('drain','排水格栅'),('vent','墙面通风箱')]],
 ...
 ('parapet_outer_ivy','女儿墙外角挂藤','02_女儿墙',lambda:ivy_corner()),
 ('parapet_inner_ivy','女儿墙内角挂藤','02_女儿墙',lambda:ivy_corner(True)),
 ('room_wall_ivy','房间实墙挂藤','03_房间墙体',lambda:ivy_wall()),
 ('room_window_ivy','房间窗墙挂藤','03_房间墙体',lambda:ivy_wall('window')),
 ('room_doorwall_ivy','房间门洞墙挂藤','03_房间墙体',lambda:ivy_wall('door')),
 ('facade_solid_ivy','标准外墙实墙挂藤','10_外墙系统',lambda:ivy_wall(external=True)),
 ('facade_window_ivy','标准外墙窗墙挂藤','10_外墙系统',lambda:ivy_wall('window',True))]
...
for slug,zh,cat,fn in specs:
    ...
    pid='ENV-ROOFTOP-REF-'+slug.upper().replace('_','-');rootob['asset_id']=pid;rootob['block_id']='rooftop';rootob['version']='v002'
    size=hi-lo
    sockets={}
    ...
    if slug.endswith('_ivy') and slug!='parapet_ivy':
        base=slug[:-4]
        item['variant_of']='ENV-ROOFTOP-REF-'+base.upper().replace('_','-')
        item['vegetation_variant']=True
        core=parts['水泥结构']
        core_lo=Vector(tuple(min(v[i] for v in core.v) for i in range(3)))
        core_hi=Vector(tuple(max(v[i] for v in core.v) for i in range(3)))
        core_center=(core_lo+core_hi)/2;core_center.z=0
        item['structural_core_offset']=list(core_center)
        if slug.startswith(('room','facade')):
            item['structural_bounds']=[5,.3,11.9]
            item['collision_bounds']=[5,.3,12];item['logical_wall_height']=12
            item['sockets']={'left':[core_center.x-2.5,core_center.y,0],'right':[core_center.x+2.5,core_center.y,0],'upper_floor':[core_center.x,core_center.y,12]}
        else:
            item['structural_bounds']=[2.5,2.5,1.8]
    if slug=='door_lamp':
        item['clear_door_leaf_size']=[3.68,.18,6.8]
        item['masonry_frame_depth']=1.1;item['lintel_depth']=1.16;item['threshold_depth']=1.55
        item['door_recess_depth']=.68
    ...
    path=O/'component_packages_v002'/cat[:2]/slug;path.mkdir(parents=True,exist_ok=True)
...
for i in range(10):
    ...
    for side in range(4):
        ...
        base='facade_window' if i%3!=0 else 'facade_solid'
        place(base+'_ivy' if i in (1,4,8) else base,x,y,-12,ang)
# room front at y=-8, back +12, X +/-10 (shared corner trim covers junction).
for i in range(4):
    x=-7.5+i*5
    place('room_doorwall_ivy' if i==3 else ('room_window_ivy' if i==1 else 'room_wall'),x,-8,0)
...
place('plant_large',4.6,-9.5,0);place('plant_small',-8.6,-8.9,0);place('plant_small',10.7,-9.5,0)
place('hvac_cabinet',1,-8.9,0);place('hvac_electric',11,-8.85,0);place('detail_vent',-8,-8.5,.2)
for x,z in [(-9,7.8),(3.5,8)]:place('ivy',x,-8.24,z)
...
cameras={
 ...
 '02_俯视结构':camera('02_俯视结构',(0,0,110),(0,0,0),73),
 '03_房间设备近景':camera('03_房间设备近景',(31,-48,38),(0,0,6),44),
 '04_外墙接口':camera('04_外墙接口',(41,-54,21),(0,-23,-4),59),
 '06_厚门框与藤蔓近景':camera('06_厚门框与藤蔓近景',(18,-25,13),(7.5,-8.6,4.4),15)}
...
lamp=bpy.data.objects.new('门灯照明_展示专用',ld);show.objects.link(lamp);lamp.location=(7.5,-9.25,7.3)
...
scene['asset_ledger']='assets/registry/ShellStorm2_美术资产台账_v002.xlsx#3D-场景通用'
