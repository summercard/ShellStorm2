"""Export clean v003 stairwell GLBs from Blender source v010."""
...
EXPORTS = (
    (
        ...
        "env_tower_stairwell_generic_9m_top3d_v003.glb",
        "ENV-TOWER-STAIRWELL-GENERIC-9M",
    ),
    (
        "Stair_Special_Rooftop_ROOT",
        "env_tower_stairwell_rooftop_9m_top3d_v003.glb",
...
def export_stairwell(root_name, filename, asset_id):
    ...
    wall_names = [obj.name for obj in descendants(root) if "EnclosureWall_" in obj.name]
    if len(wall_names) != 4:
        raise RuntimeError(f"{root_name} has {len(wall_names)} enclosure walls: {wall_names}")
    ...
    root["asset_version"] = "v003"
    root["blender_source_version"] = "v010"
    ...
    print(f"EXPORTED {root_name} -> {output_path} objects={len(selected)} walls=4")


for export_spec in EXPORTS:
    export_stairwell(*export_spec)

print(f"STAIRWELL_V003_EXPORT_OK count={len(EXPORTS)} output={OUTPUT_DIR}")
