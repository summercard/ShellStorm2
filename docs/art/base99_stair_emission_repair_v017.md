...
2026-09-05。制作源：`base_facility_runtime_layout_hq_v017.blend`；优化源：`export/v017/base_facility_runtime_layout_hq-v017-structural.blend`。四项结构包装使用 `export/v017/` 目录内的 `*_visual_top3d_v002.glb`。

> 2026-09-07 修订：本文件原名 `_v021.md`，原将优化源写作 `v021/base_facility_runtime_layout_hq_v021_structural.blend`。经核实，所谓的 v021/v022 优化源实际是 v017 源的分组导出（命名误用版本号），现已统一迁移至 `export/v017/` 目录、文件名改为 `-v017-<类型>.blend` 连字符规范。本文同步更名并更正全部路径引用。
...
V017 结构输出将自发光与主体分为独立网格。四包共7个输出网格、31,450三角面，逐面PaletteUV安全区和有效面积检查全部通过。未以整个源场景的历史校验问题替代本次输出验收。

## 验收

- `verify_base99_stair_walkable_v017`：真实Player3D在正式基地完成一楼→两跑→阁楼，以及反向通行；场景色盘发光混合检查通过。
