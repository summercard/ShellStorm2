...
**状态判定修订（更新至2026-09-12）：** 审计发现不再统一称为“不匹配”或“错误”。D01、D03–D07、D13完成文档对齐工程后，44项跟踪现分为10项已一致、10项开发中·部分完成、6项工程/文档不匹配、12项待设计／核验、6项文档缺失；10项已处理、34项未处理。已有正式子链路但尚未完成全部目标契约的项目属于开发进度；下文A01–A10保留问题域和风险证据，不代表每个未完成目标都是代码错误。逐项分类以[结构化差异清单](evidence/discrepancy_inventory_20260912.json)为准。
...
**后续状态（2026-09-12）：** 差异表E03已修复并通过`verify_extraction_points_spend_transaction`。普通资源扣款现在只在写盘和回读成功后返回成功，失败恢复余额；revision冲突保留重新加载的权威档案。下列故障探针仍作为修复前证据保留。工坊两步升级、撤离／死亡结算及`set_blueprint_tier`、`add_extraction_points`的独立保存语义尚未收口，因此A02整体仍未关闭。
...
**后续状态（2026-09-12）：** 下表保留原审计证据。D01、D03–D07、D13已按用户决定完成文档对齐工程：弹药名称、5个电池/手电正式ID与状态、基地商店购买去向已经同步。尚未关闭的是D02弹药堆叠上限、D08–D11价格/字段语义和D12 Boss专钥匙目标链路。

| 内容 | 表格位置/值 | 实际运行时 | 影响 |
|---|---|---|---|
| 弹药 | 掉落物品B18“弹药包”、G18堆叠5 | item_ammo_pack“通用弹药”、stack_max=999 | 按表制作/平衡会得到不同背包行为 |
| 电池/手电模块ID | A42:A46为battery_l、cell_pack、flashlight_module_basic/advanced/efficient | item_battery_l、item_cell_pack、item_flashlight_basic/advanced/efficient | 5个正式运行ID在表中缺行，对应设计ID没有显式映射 |
| 大型近战买卖 | 掉落物品P47:Q48为空且无公式 | 断刃165/83、战斧240/120 | 成交字段来源不完整；未上架不等于不能卖 |
...
另外检测到普通钥匙价格与小电池subtype缺省差异：当前消费者可能有默认值，归为schema/字段完整性问题，不直接认定玩家行为错误。
...
| 后处理/训练场/工坊 | 代码或测试有，但完整功能契约/逐功能开发记录不足 | 索引明确列为缺口，不补造历史 |
...
| 5 | 补齐剧情、复活、后处理、商店/工坊/靶场功能契约 | 对应功能ID、目标版本与修订 | 每项有唯一设计依据、数据/失败契约和真实开发记录；目标内容继续与已实现分离 |
