...
| WORLD | `FloorPlanGenerator`纯计划→`RoomGraphRuntime`查询→`Dungeon3D/TowerDescent3D`装配 | 计划/查询可独立，生命周期不足 | 9572行父子编排、私有字段依赖、开门/卸载未等待存档成功 |
...
| TIME | `WorldTimeDomain`→`GameTimeManager`→BaseData时间→太阳/HUD/能源 | 算法可独立，接入部分 | 规则仍为代码常量；联动、持久化测试需进入明确模块验收集 |
...
| TRAINING | `TrainingRange3D`→共用武器目录和Player3D→靶标统计 | 场景可独立启动 | 无完整专项设计；共用Autoload不等于隔离会话 |
...
| PLAYER-LIGHT | 手电、电池、光照/可见性 | [03](03_技术施工_玩家与操作.md)、[15](15_技术施工_时间日夜与基地能源.md) | `PlayerFlashlight3D/PlayerVision3D`→ItemUseHandler | `verify_3d_flashlight_charge_flow` | 设计有；数据ID映射与灯光测试失配 |
...
| WORLD-GATE | 到达门、Boss门、楼梯 | [05](05_技术施工_关卡生成与爬楼.md)、[09](09_技术施工_存档结算与复活.md) | `TowerDescent3D`→FloorBundle→RoomDoor3D | `verify_arrival_gate_floor_bundle_flow` | 正常路径有测试；写盘失败仍开门已复现 |
| WORLD-SEGMENT | 隔离间、区段卸载、永久遗失 | [05](05_技术施工_关卡生成与爬楼.md) | `TowerDescent3D._finalize_airlock_commit` | `verify_three_segment_tower_generation_flow` | 有目标与正常路径；缺提交故障/重启恢复专项闭环 |
...
| BASE-SHOP | 基地购买、出售、保险柜转移 | [07](07_技术施工_基地设施.md)、[09](09_技术施工_存档结算与复活.md) | BaseManager事务→BaseShopService→ItemRegistry | `verify_base_shop_save_flow`、`verify_tower_facility_inventory_binding` | 已有幂等/回滚；普通扣款接口未同步同标准 |
| BASE-WORKSHOP | 蓝图升级与手电模块 | [07](07_技术施工_基地设施.md) | `src/ui/WorkshopMenu.gd`→BaseManager | `verify_base_facility_framework`（仅相邻覆盖） | 缺独立升级原子事务/失败专项与逐功能开发记录 |
| RUN-MERCHANT | 局内商人、消费与回退 | [04](04_技术施工_战斗与局内成长.md)（上级） | `src/ui/MerchantUI.gd`→GameManager/Inventory | `verify_full_3d_game_flow`（综合） | 无完整独立契约与逐功能记录，UI持有交易编排 |
| SAVE-PROFILE | 总档封套、校验、迁移与复位 | [09](09_技术施工_存档结算与复活.md) | `BaseData/ProfileSaveService/AtomicJsonStore` | `verify_pause_game_save_reset_flow`、`verify_base_shop_save_flow` | 有规范与历史；文件失败/备份链仍需补故障矩阵 |
...
| TIME-ENERGY | 权威时间、日夜与恢复能源 | [15](15_技术施工_时间日夜与基地能源.md) | WorldTimeDomain/GameTimeManager/BaseEnergyService | `verify_main_entry_realtime_sun_flow`、`verify_base_overhaul_flow` | 有规范；缺统一数据配置与失败记录链 |
...
| TRAINING-RANGE | 独立靶场与武器预览 | [04](04_技术施工_战斗与局内成长.md)（上级） | `src/training3d/TrainingRange3D.gd` | `verify_training_range_3d_flow` | 实现/测试有，完整独立设计与逐功能日志不足 |
...
- NARRATIVE/RUN-REVIVE/RUN-MERCHANT/TRAINING/GRAPHICS-POSTFX/BASE-WORKSHOP：不得因能找到相关大章节就认定设计及开发日志完整；优先补功能级契约和真实交付记录。
