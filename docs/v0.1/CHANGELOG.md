...
## 2026-08-13｜玩家九宫格上下层楼板保光

- 修复非活动物理层整块楼板隐藏后太阳直穿室内的问题：以镜头地面投影所在5m格为中心，正上层和正下层分别保留约50×50m（10×10）局部楼板，楼梯洞口仍保持真实空洞。
- 局部补丁只在镜头跨越5m格时更新；当前活动层继续完整渲染，远层房间、外墙和处理逻辑继续卸载，额外渲染严格限制为最多200个MultiMesh实例。
- 塔楼诊断快照新增补丁层号、层数、镜头中心与实例数；独立核心专项覆盖镜头跨格更新、完整层去重、99层上下层补丁、单层≤100和总计≤200契约。
- 新增专项及楼层生成、运行时性能、网格覆盖、光照墙体、Metal塔楼视觉共6项通过；全量核心汇总被工作区既有两个未跟踪旧2D场景挡在结构门禁，未删除用户文件且未虚报全量通过。
