...
- 修复99层角色视觉陷入地板、而100层表现正常的问题：99层普通板与铆钉板GLB均采用底面原点，但两者高度分别约0.30米与0.395米；旧逻辑共用固定Y，导致较厚模型从承重面向上包住角色。
- 99层地板MultiMesh现按每个正式GLB的真实AABB顶面独立计算Y偏移，将两种地板统一到与100层通用地砖相差1.5厘米的表现高度；继续由原`TowerFloorStage3D/FloorSupport`承担碰撞，不修改角色胶囊、碰撞层、楼梯或100层地板。
- 新增`verify_base99_floor_player_collision_flow`，覆盖普通/铆钉地板同高、与100层表现高度一致、99层中心承重碰撞存在且永久驻留。
