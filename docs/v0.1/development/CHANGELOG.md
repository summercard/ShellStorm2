...
## 2026-09-16｜局内关卡01入口安全房正式美术 v003 与 Godot 接入

- 按参考图将 `局内关卡01_白模_入口安全房_15x15m_v002` 制作并拆分为33个正式美术资产包，输出33个GLB、33个独立PackedScene和1个整房总装配；源场景56个输出网格在优化后为51,757三角面，维持4个共享材质角色且GLB内嵌图片/纹理均为0。
- 总装配保持15×15×11.9m、底面贴地和Godot `-Z`正面方向；南门视觉净宽2.2m、门楣底高2.8m，玩法门仍保持2.2×2.5m。视觉包不生成覆盖式碰撞，承重与门体权威继续归`TowerDescent3D.FloorSupport + RoomDoor3D`。
- `floor_01_entry`仅在`room_door_world_*`布局冻结后安装正式美术，并在初始启动、撤退重建和楼层提交后刷新；旧程序化地砖、墙体与门洞视觉同步隐藏，安装幂等。
- 共享低亮多巴胺色盘、独立加载、整房包围盒、门洞净空、启动/重载/楼层提交和真实渲染回归均纳入专项；v002白盒与制作前工作区Blend保留回滚。
- 详见[独立交付记录](2026-09-16_entry_safe_room_art_v003_godot_import.md)。
