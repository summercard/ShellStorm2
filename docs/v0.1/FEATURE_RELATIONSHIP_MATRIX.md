...
工程版本：`0.1.0`；表格修订：r3；核对日期：2026-09-23；新拉取基线：`622d6c4a`。执行方法见[计划](FEATURE_RELATIONSHIP_PLAN.md)，功能的权威 Owner/设计/记录/验收路径见[模块索引](MODULE_INDEX.md)与[功能注册表](feature_registry.json)。本表是跟踪视图，状态栏不覆盖注册表的实现状态。
...
| WORLD-LOOT | 清房/搜索/剧情`scene.spawn_item` → RewardCoordinator → 房间地面物/背包 | 关系待收口 | 04/05/08已定位；开场枪的房间快照、实例ID与重启恢复已专项核对，旧item桥仍在 | 其他地面物逐内容验收；迁移旧字典桥 | P1 | 持续维护 |
