...
- <span style="color:#2E7D32">**[已实装｜2026-08-11光照战斗与AI感知]** 黑暗保持基线；人工灯光下移动速度和攻击频率均为70%；太阳每秒扣最大生命2%。临时三态标识复用头顶生命条纹理。全局 `MonsterAIManager` 与独立 `MonsterVisionSystem3D` 已接管目标授权、120°/13.5m视野、遮挡、记忆、手电来源追踪、大灯搜索和避光策略；`Enemy3D` 不再无条件扫描最近玩家。</span>
...
| `artificial_light` 受到灯光 | 现有值70% | 现有值70% | 无 | 青蓝色双段标记 |
| `sunlight` 受到太阳光 | 现有值100% | 现有值100% | 每秒损失最大生命2% | 暖黄色三段标记 |

“攻击频率70%”按时间频率计算，等价于有效攻击间隔变为原来的 `1 / 0.70`；不能把攻击间隔本身乘0.70，否则会错误加快攻击。移动与攻击修饰只能在计算最终值时应用，禁止反复乘写基础 Profile，避免灯光边缘切换造成永久降速。太阳伤害为独立环境伤害通道，每1秒结算一次、最低1点，可击杀普通怪和 Boss，并正常触发生命条、伤害数字、死亡、掉落和清房事务；不复用武器 DOT，避免覆盖燃烧等持续伤害。

临时验收 UI 不新增每怪场景节点：复用现有头顶生命条纹理，在生命条上沿绘制三态颜色和一/二/三段编码。状态切换立即重绘；发布前可由统一调试开关关闭，但测试快照必须保留 `illumination_ui_state / color / segment_count`。
...
视野基线为13.5m、水平夹角120°，同时满足距离、朝向与静态世界物理遮挡才算看见玩家。已警觉怪物保留1.15秒最后已知位置记忆；未看见玩家时禁止直接把最近玩家世界坐标写入搜索目标。视野射线排除怪物自身，目标使用躯干采样点，墙体、关闭的门、楼板等静态碰撞可遮挡。
...
- [x] 普通灯光状态的最终移动速度与攻击频率均为黑暗基线70%，反复切换后可准确恢复100%。
- [x] 太阳直射每秒扣最大生命2%，进入投影立刻停止，死亡仍完成掉落与清房事务。
- [x] 三态头顶标识复用现有生命条节点，颜色和一/二/三段编码与快照一致。
...
7. <span style="color:#2E7D32">**[已完成]** 实现三态战斗效果、无新增节点的临时头顶标识、独立 `MonsterAIManager` 与 `MonsterVisionSystem3D`。</span>
